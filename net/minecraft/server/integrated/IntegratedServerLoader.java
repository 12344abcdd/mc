...
public class IntegratedServerLoader {
   ...
   private LevelStorage.Session createSession(String levelName) {
      try {
         ...
         this.client.setScreen(SymlinkWarningScreen.world(() -> this.client.setScreen((Screen)null)));
         return null;
      }
   }
   ...
   private Object load(SaveLoading.DataPacks dataPacks, SaveLoading.LoadContextSupplier loadContextSupplier, SaveLoading.SaveApplierFactory saveApplierFactory) throws Exception {
      SaveLoading.ServerConfig serverConfig = new SaveLoading.ServerConfig(dataPacks, RegistrationEnvironment.INTEGRATED, 2);
   ...
   private void start(LevelStorage.Session session, LevelSummary summary, Dynamic levelProperties, Runnable onCancel) {
      if (!summary.isVersionAvailable()) {
         session.tryClose();
         this.client.setScreen(new NoticeScreen(onCancel, Text.translatable("selectWorld.incompatible.title").withColor(-65536), Text.translatable("selectWorld.incompatible.description", new Object[]{summary.getVersion()})));
      } else {
