...
public class PeriodicNotificationManager extends SinglePreparationResourceReloader implements AutoCloseable {
   ...
   protected void apply(Map map, ResourceManager resourceManager, Profiler profiler) {
      List<Entry> list = (List)map.entrySet().stream().filter((entry) -> (Boolean)this.countryPredicate.apply((String)entry.getKey())).map(Map.Entry::getValue).flatMap(Collection::stream).collect(Collectors.toList());
      if (list.isEmpty()) {
         this.cancelTimer();
   ...
   static {
      CODEC = Codec.unboundedMap(Codec.STRING, RecordCodecBuilder.create((instance) -> instance.group(Codec.LONG.optionalFieldOf("delay", 0L).forGetter(Entry::delay), Codec.LONG.fieldOf("period").forGetter(Entry::period), Codec.STRING.fieldOf("title").forGetter(Entry::title), Codec.STRING.fieldOf("message").forGetter(Entry::message)).apply(instance, Entry::new)).listOf());
      ...
      public void run() {
         ...
         for(Entry entry : this.entries) {
            if (l >= entry.delay) {
               ...
               if (n != o) {
                  this.client.execute(() -> SystemToast.add(MinecraftClient.getInstance().getToastManager(), SystemToast.Type.PERIODIC_NOTIFICATION, Text.translatable(entry.title, new Object[]{n}), Text.translatable(entry.message, new Object[]{n})));
                  return;
               }
            }
         }
