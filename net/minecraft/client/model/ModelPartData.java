...
public class ModelPartData {
   ...
   public ModelPart createPart(int textureWidth, int textureHeight) {
      Object2ObjectArrayMap<String, ModelPart> object2ObjectArrayMap = (Object2ObjectArrayMap)this.children.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, (entry) -> ((ModelPartData)entry.getValue()).createPart(textureWidth, textureHeight), (modelPartx, modelPart2) -> modelPartx, Object2ObjectArrayMap::new));
      List<ModelPart.Cuboid> list = (List)this.cuboidData.stream().map((modelCuboidData) -> modelCuboidData.createCuboid(textureWidth, textureHeight)).collect(ImmutableList.toImmutableList());
