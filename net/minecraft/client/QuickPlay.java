...
public class QuickPlay {
   ...
   private static void startSingleplayer(MinecraftClient client, String levelName) {
      if (!client.getLevelStorage().levelExists(levelName)) {
         ...
         client.createIntegratedServerLoader().start((String)levelName, () -> client.setScreen(new TitleScreen()));
      }
   }
   ...
   private static void startRealms(MinecraftClient client, RealmsClient realmsClient, String realmId) {
      ...
      RealmsServer realmsServer = (RealmsServer)realmsServerList.servers.stream().filter((server) -> server.id == l).findFirst().orElse((Object)null);
      if (realmsServer == null) {
         ...
         client.setScreen(new RealmsLongRunningMcoTaskScreen(titleScreen, new LongRunningTask[]{realmsPrepareConnectionTask}));
      }
   }
}
