...
public record TrueTypeFontLoader(Identifier location, float size, float oversample, Shift shift, String skip) implements FontLoader {
      ...
      try {
         ...
         try {
            ...
            synchronized(FreeTypeUtil.LOCK) {
               ...
               var21 = new TrueTypeFont(byteBuffer, fT_Face, this.size, this.oversample, this.shift.x, this.shift.y, this.skip);
            }
   ...
   static {
      SKIP_CODEC = Codec.withAlternative(Codec.STRING, Codec.STRING.listOf(), (chars) -> String.join("", chars));
   ...
   public static record Shift(float x, float y) {
      ...
      public static final Codec CODEC = Codec.floatRange(-512.0F, 512.0F).listOf().comapFlatMap((floatList) -> Util.decodeFixedLengthList(floatList, 2).map((floatListx) -> new Shift((Float)floatListx.get(0), (Float)floatListx.get(1))), (shift) -> List.of(shift.x, shift.y));
   }
}
