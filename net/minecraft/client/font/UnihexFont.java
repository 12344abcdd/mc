...
public class UnihexFont implements Font {
   ...
   static void readLines(InputStream stream, BitmapGlyphConsumer callback) throws IOException {
      ...
      while(true) {
         ...
         switch (l) {
            case 32 -> var10000 = UnihexFont.FontImage8x16.read(i, byteList);
            case 64 -> var10000 = UnihexFont.FontImage16x16.read(i, byteList);
            case 96 -> var10000 = UnihexFont.FontImage32x16.read24x16(i, byteList);
            case 128 -> var10000 = UnihexFont.FontImage32x16.read32x16(i, byteList);
   ...
   @Environment(EnvType.CLIENT)
   static record DimensionOverride(int from, int to, Dimensions dimensions) {
      final int from;
      final int to;
      final Dimensions dimensions;
      private static final Codec NON_VALIDATED_CODEC = RecordCodecBuilder.create((instance) -> instance.group(Codecs.CODEPOINT.fieldOf("from").forGetter(DimensionOverride::from), Codecs.CODEPOINT.fieldOf("to").forGetter(DimensionOverride::to), UnihexFont.Dimensions.MAP_CODEC.forGetter(DimensionOverride::dimensions)).apply(instance, DimensionOverride::new));
      public static final Codec CODEC;

      static {
         CODEC = NON_VALIDATED_CODEC.validate((override) -> override.from >= override.to ? DataResult.error(() -> "Invalid range: [" + override.from + ";" + override.to + "]") : DataResult.success(override));
      }
   }
   ...
   public static record Dimensions(int left, int right) {
      final int left;
      final int right;
      public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(Codec.INT.fieldOf("left").forGetter(Dimensions::left), Codec.INT.fieldOf("right").forGetter(Dimensions::right)).apply(instance, Dimensions::new));
      public static final Codec CODEC;
   ...
   public static class Loader implements FontLoader {
      public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(Identifier.CODEC.fieldOf("hex_file").forGetter((loader) -> loader.sizes), UnihexFont.DimensionOverride.CODEC.listOf().fieldOf("size_overrides").forGetter((loader) -> loader.overrides)).apply(instance, Loader::new));
      ...
      private UnihexFont loadHexFile(InputStream stream) throws IOException {
         GlyphContainer<BitmapGlyph> glyphContainer = new GlyphContainer((ix) -> new BitmapGlyph[ix], (rows) -> new BitmapGlyph[rows][]);
         ...
         try {
            ...
            GlyphContainer<UnicodeTextureGlyph> glyphContainer2 = new GlyphContainer((ix) -> new UnicodeTextureGlyph[ix], (ix) -> new UnicodeTextureGlyph[ix][]);
   ...
   @Environment(EnvType.CLIENT)
   static record UnicodeTextureGlyph(BitmapGlyph contents, int left, int right) implements Glyph {
