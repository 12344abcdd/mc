...
public class TextHandler {
   ...
   public Style getStyleAt(StringVisitable text, int x) {
      ...
      return (Style)text.visit((style, textx) -> TextVisitFactory.visitFormatted(textx, style, widthLimitingVisitor) ? Optional.empty() : Optional.of(style), Style.EMPTY).orElse((Object)null);
   }
   ...
   public List wrapLines(StringVisitable text, int maxWidth, Style style) {
      List<StringVisitable> list = Lists.newArrayList();
      this.wrapLines(text, maxWidth, style, (BiConsumer)((textx, lastLineWrapped) -> list.add(textx)));
      return list;
   }

   public List wrapLines(StringVisitable text, int maxWidth, Style style, StringVisitable wrappedLinePrefix) {
      List<StringVisitable> list = Lists.newArrayList();
      this.wrapLines(text, maxWidth, style, (BiConsumer)((textx, lastLineWrapped) -> list.add(lastLineWrapped ? StringVisitable.concat(new StringVisitable[]{wrappedLinePrefix, textx}) : textx)));
      return list;
   }
