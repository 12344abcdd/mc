...
public class ConduitBlockEntityRenderer implements BlockEntityRenderer {
   ...
   public void render(ConduitBlockEntity conduitBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
      ...
      if (!conduitBlockEntity.isActive()) {
         ...
         matrixStack.multiply((new Quaternionf()).rotationY(h * ((float)Math.PI / 180F)));
         this.conduitShell.render(matrixStack, vertexConsumer, i, j);
         matrixStack.pop();
      } else {
         float h = conduitBlockEntity.getRotation(f) * (180F / (float)Math.PI);
         ...
         matrixStack.multiply((new Quaternionf()).rotationAxis(h * ((float)Math.PI / 180F), vector3f));
         ...
         matrixStack.translate(0.5F, 0.5F, 0.5F);
         if (l == 1) {
            matrixStack.multiply((new Quaternionf()).rotationX(((float)Math.PI / 2F)));
         } else if (l == 2) {
            matrixStack.multiply((new Quaternionf()).rotationZ(((float)Math.PI / 2F)));
         }
         ...
         matrixStack.multiply((new Quaternionf()).rotationYXZ(m * ((float)Math.PI / 180F), camera.getPitch() * ((float)Math.PI / 180F), (float)Math.PI));
         float n = 1.3333334F;
   ...
   static {
      BASE_TEXTURE = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/base"));
      CAGE_TEXTURE = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/cage"));
      WIND_TEXTURE = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/wind"));
      WIND_VERTICAL_TEXTURE = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/wind_vertical"));
      OPEN_EYE_TEXTURE = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/open_eye"));
      CLOSED_EYE_TEXTURE = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/closed_eye"));
   }
}
