...
public class SignBlockEntityRenderer implements BlockEntityRenderer {
   ...
   private static final Vec3d TEXT_OFFSET = new Vec3d((double)0.0F, (double)0.33333334F, (double)0.046666667F);
   private final Map typeToModel;
   private final TextRenderer textRenderer;

   public SignBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
      this.typeToModel = (Map)WoodType.stream().collect(ImmutableMap.toImmutableMap((signType) -> signType, (signType) -> new SignModel(ctx.getLayerModelPart(EntityModelLayers.createSign(signType)))));
   ...
   public void render(SignBlockEntity signBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
      ...
      SignModel signModel = (SignModel)this.typeToModel.get(woodType);
   ...
   void renderText(BlockPos pos, SignText signText, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int lineHeight, int lineWidth, boolean front) {
      ...
      for(int m = 0; m < 4; ++m) {
         ...
         if (bl) {
            ...
            this.textRenderer.draw((OrderedText)orderedText, f, (float)(m * lineHeight - j), k, false, matrices.peek().getPositionMatrix(), vertexConsumers, TextRenderer.TextLayerType.POLYGON_OFFSET, 0, l);
         }
      }
   ...
   public static int getColor(SignText sign) {
      ...
      if (i == DyeColor.BLACK.getSignColor() && sign.isGlowing()) {
         ...
         double d = 0.4;
         int j = (int)((double)Argb.getRed(i) * 0.4);
         int k = (int)((double)Argb.getGreen(i) * 0.4);
         int l = (int)((double)Argb.getBlue(i) * 0.4);
         return Argb.getArgb(0, j, k, l);
      }
   }
