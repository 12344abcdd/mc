...
public class BakedQuadFactory {
   ...
   private static final float MIN_SCALE = 1.0F / (float)Math.cos((double)((float)Math.PI / 8F)) - 1.0F;
   private static final float MAX_SCALE = 1.0F / (float)Math.cos((double)((float)Math.PI / 4F)) - 1.0F;
   ...
   public static ModelElementTexture uvLock(ModelElementTexture texture, Direction orientation, AffineTransformation rotation) {
      ...
      int s = Math.floorMod(-((int)Math.round(Math.toDegrees(Math.atan2((double)vector3f.y(), (double)vector3f.x())) / (double)90.0F)) * 90, 360);
   ...
   private void rotateVertex(Vector3f vector, @Nullable net.minecraft.client.render.model.json.ModelRotation rotation) {
      if (rotation != null) {
         ...
         Quaternionf quaternionf = (new Quaternionf()).rotationAxis(rotation.angle() * ((float)Math.PI / 180F), vector3f);
         if (rotation.rescale()) {
