...
public class WeightedBakedModel implements BakedModel {
   ...
   public List getQuads(@Nullable BlockState state, @Nullable Direction face, Random random) {
      return (List)Weighting.getAt(this.models, Math.abs((int)random.nextLong()) % this.totalWeight).map((present) -> ((BakedModel)present.data()).getQuads(state, face, random)).orElse(Collections.emptyList());
   }
