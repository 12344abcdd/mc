...
public class BakedModelManager implements ResourceReloader, AutoCloseable, FabricBakedModelManager {
   ...
   public final CompletableFuture reload(ResourceReloader.Synchronizer synchronizer, ResourceManager manager, Profiler prepareProfiler, Profiler applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
      ...
      CompletableFuture<ModelLoader> completableFuture3 = completableFuture.thenCombineAsync(completableFuture2, (jsonUnbakedModels, blockStates) -> new ModelLoader(this.colorMap, prepareProfiler, jsonUnbakedModels, blockStates), prepareExecutor);
      ...
      CompletableFuture var10000 = CompletableFuture.allOf((CompletableFuture[])Stream.concat(map.values().stream(), Stream.of(completableFuture3)).toArray((i) -> new CompletableFuture[i])).thenApplyAsync((void1) -> this.bake(prepareProfiler, (Map)map.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, (entry) -> (SpriteAtlasManager.AtlasPreparation)((CompletableFuture)entry.getValue()).join())), (ModelLoader)completableFuture3.join()), prepareExecutor).thenCompose((result) -> result.readyForUpload.thenApply((void_) -> result));
      Objects.requireNonNull(synchronizer);
      return var10000.thenCompose(synchronizer::whenPrepared).thenAcceptAsync((result) -> this.upload(result, applyProfiler), applyExecutor);
   }

   private static CompletableFuture reloadModels(ResourceManager resourceManager, Executor executor) {
      return CompletableFuture.supplyAsync(() -> ModelLoader.MODELS_FINDER.findResources(resourceManager), executor).thenCompose((models) -> {
         ...
         return Util.combineSafe(list).thenApply((modelsx) -> (Map)modelsx.stream().filter(Objects::nonNull).collect(Collectors.toUnmodifiableMap(Pair::getFirst, Pair::getSecond)));
      });
   }

   private static CompletableFuture reloadBlockStates(ResourceManager resourceManager, Executor executor) {
      return CompletableFuture.supplyAsync(() -> BlockStatesLoader.FINDER.findAllResources(resourceManager), executor).thenCompose((blockStates) -> {
         ...
         for(Map.Entry entry : blockStates.entrySet()) {
            list.add(CompletableFuture.supplyAsync(() -> {
               ...
               List<BlockStatesLoader.SourceTrackedData> list2 = new ArrayList(list.size());
               ...
               for(Resource resource : list) {
                  try {
                     ...
                     LOGGER.error("Failed to load blockstate {} from pack {}", new Object[]{entry.getKey(), resource.getPackId(), exception});
                  }
               }
         ...
         return Util.combineSafe(list).thenApply((blockStatesx) -> (Map)blockStatesx.stream().filter(Objects::nonNull).collect(Collectors.toUnmodifiableMap(Pair::getFirst, Pair::getSecond)));
      });
   }
   ...
   private BakingResult bake(Profiler profiler, Map preparations, ModelLoader modelLoader) {
      ...
      multimap.asMap().forEach((modelIdentifier, spriteIds) -> LOGGER.warn("Missing textures in model {}:\n{}", modelIdentifier, spriteIds.stream().sorted(SpriteIdentifier.COMPARATOR).map((spriteIdentifier) -> {
            String var10000 = String.valueOf(spriteIdentifier.getAtlasId());
            return "    " + var10000 + ":" + String.valueOf(spriteIdentifier.getTextureId());
      ...
      CompletableFuture<Void> completableFuture = CompletableFuture.allOf((CompletableFuture[])preparations.values().stream().map(SpriteAtlasManager.AtlasPreparation::whenComplete).toArray((i) -> new CompletableFuture[i]));
      profiler.pop();
      profiler.endTick();
   ...
   @Environment(EnvType.CLIENT)
   static record BakingResult(ModelLoader modelLoader, BakedModel missingModel, Map modelCache, Map atlasPreparations, CompletableFuture readyForUpload) {
