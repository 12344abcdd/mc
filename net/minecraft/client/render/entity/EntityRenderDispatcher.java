...
public class EntityRenderDispatcher implements SynchronousResourceReloader {
   ...
   public EntityRenderer getRenderer(Entity entity) {
      if (entity instanceof AbstractClientPlayerEntity abstractClientPlayerEntity) {
         ...
         EntityRenderer<? extends PlayerEntity> entityRenderer = (EntityRenderer)this.modelRenderers.get(model);
   ...
   public void render(Entity entity, double x, double y, double z, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
      ...
      try {
         ...
         if ((Boolean)this.gameOptions.getEntityShadows().getValue() && this.renderShadows && !entity.isInvisible()) {
            ...
            if (g > 0.0F) {
               ...
               float i = (float)(((double)1.0F - h / (double)256.0F) * (double)entityRenderer.shadowOpacity);
               if (i > 0.0F) {
   ...
   private static void renderServerSideHitbox(MatrixStack matrices, Entity entity, VertexConsumerProvider vertexConsumers) {
      ...
      if (entity2 == null) {
         DebugRenderer.drawString(matrices, vertexConsumers, "Missing", entity.getX(), entity.getBoundingBox().maxY + (double)1.5F, entity.getZ(), -65536);
      } else {
         matrices.push();
   ...
   private static void renderHitbox(MatrixStack matrices, VertexConsumer vertices, Entity entity, float tickDelta, float red, float green, float blue) {
      ...
      if (entity2 != null) {
         ...
         WorldRenderer.drawBox(matrices, vertices, vec3d.x - (double)k, vec3d.y, vec3d.z - (double)k, vec3d.x + (double)k, vec3d.y + (double)0.0625F, vec3d.z + (double)k, 1.0F, 1.0F, 0.0F, 1.0F);
      }

      drawVector(matrices, vertices, new Vector3f(0.0F, entity.getStandingEyeHeight(), 0.0F), entity.getRotationVec(tickDelta).multiply((double)2.0F), -16776961);
   }
         ...
         if (blockState.isFullCube(chunk, blockPos)) {
            ...
            if (!voxelShape.isEmpty()) {
               ...
               if (g >= 0.0F) {
                  ...
                  int i = Argb.getArgb(MathHelper.floor(g * 255.0F), 255, 255, 255);
