...
public class MinecartEntityRenderer extends EntityRenderer {
   ...
   public void render(AbstractMinecartEntity abstractMinecartEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
      ...
      if (vec3d != null) {
         Vec3d vec3d2 = abstractMinecartEntity.snapPositionToRailWithOffset(d, e, m, (double)0.3F);
         Vec3d vec3d3 = abstractMinecartEntity.snapPositionToRailWithOffset(d, e, m, (double)-0.3F);
         if (vec3d2 == null) {
            vec3d2 = vec3d;
         }
         ...
         matrixStack.translate(vec3d.x - d, (vec3d2.y + vec3d3.y) / (double)2.0F - e, vec3d.z - m);
         Vec3d vec3d4 = vec3d3.add(-vec3d2.x, -vec3d2.y, -vec3d2.z);
         if (vec3d4.length() != (double)0.0F) {
            vec3d4 = vec3d4.normalize();
            f = (float)(Math.atan2(vec3d4.z, vec3d4.x) * (double)180.0F / Math.PI);
            o = (float)(Math.atan(vec3d4.y) * (double)73.0F);
         }
      }
   ...
   public Identifier getTexture(AbstractMinecartEntity abstractMinecartEntity) {
      return TEXTURE;
   }
