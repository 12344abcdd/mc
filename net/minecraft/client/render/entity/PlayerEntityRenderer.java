...
public class PlayerEntityRenderer extends LivingEntityRenderer {
   ...
   public Vec3d getPositionOffset(AbstractClientPlayerEntity abstractClientPlayerEntity, float f) {
      return abstractClientPlayerEntity.isInSneakingPose() ? new Vec3d((double)0.0F, (double)(abstractClientPlayerEntity.getScale() * -2.0F) / (double)16.0F, (double)0.0F) : super.getPositionOffset(abstractClientPlayerEntity, f);
   }
   ...
   protected void renderLabelIfPresent(AbstractClientPlayerEntity abstractClientPlayerEntity, Text text, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, float f) {
      ...
      if (d < (double)100.0F) {
         ...
         if (scoreboardObjective != null) {
            ...
            super.renderLabelIfPresent(abstractClientPlayerEntity, Text.empty().append(text2).append(ScreenTexts.SPACE).append(scoreboardObjective.getDisplayName()), matrixStack, vertexConsumerProvider, i, f);
   ...
   protected void setupTransforms(AbstractClientPlayerEntity abstractClientPlayerEntity, MatrixStack matrixStack, float f, float g, float h, float i) {
      ...
      if (abstractClientPlayerEntity.isFallFlying()) {
         ...
         super.setupTransforms(abstractClientPlayerEntity, matrixStack, f, g, h, i);
         float l = abstractClientPlayerEntity.isTouchingWater() ? -90.0F - k : -90.0F;
