...
public class ItemFrameEntityRenderer extends EntityRenderer {
   ...
   public Vec3d getPositionOffset(ItemFrameEntity itemFrameEntity, float f) {
      return new Vec3d((double)((float)itemFrameEntity.getHorizontalFacing().getOffsetX() * 0.3F), (double)-0.25F, (double)((float)itemFrameEntity.getHorizontalFacing().getOffsetZ() * 0.3F));
   }
   ...
   protected boolean hasLabel(ItemFrameEntity itemFrameEntity) {
      if (MinecraftClient.isHudEnabled() && !itemFrameEntity.getHeldItemStack().isEmpty() && itemFrameEntity.getHeldItemStack().contains(DataComponentTypes.CUSTOM_NAME) && this.dispatcher.targetedEntity == itemFrameEntity) {
   ...
   protected void renderLabelIfPresent(ItemFrameEntity itemFrameEntity, Text text, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, float f) {
