...
public class VexEntityModel extends SinglePartEntityModel implements ModelWithArms {
   ...
   public void setAngles(VexEntity vexEntity, float f, float g, float h, float i, float j) {
      ...
      float k = MathHelper.cos(h * 5.5F * ((float)Math.PI / 180F)) * 0.1F;
      ...
      this.leftWing.yaw = 1.0995574F + MathHelper.cos(h * 45.836624F * ((float)Math.PI / 180F)) * ((float)Math.PI / 180F) * 16.2F;
   ...
   private void translateForHand(MatrixStack matrices, boolean mainHand) {
      if (mainHand) {
         matrices.translate((double)0.046875F, (double)-0.15625F, (double)0.078125F);
      } else {
         matrices.translate((double)-0.046875F, (double)-0.15625F, (double)0.078125F);
      }
