...
public class SilverfishEntityModel extends SinglePartEntityModel {
   ...
   public static TexturedModelData getTexturedModelData() {
      ...
      for(int i = 0; i < 7; ++i) {
         modelPartData.addChild(getSegmentName(i), ModelPartBuilder.create().uv(SEGMENT_SIZES[i][0], SEGMENT_SIZES[i][1]).cuboid((float)SEGMENT_LOCATIONS[i][0] * -0.5F, 0.0F, (float)SEGMENT_LOCATIONS[i][2] * -0.5F, (float)SEGMENT_LOCATIONS[i][0], (float)SEGMENT_LOCATIONS[i][1], (float)SEGMENT_LOCATIONS[i][2]), ModelTransform.pivot(0.0F, (float)(24 - SEGMENT_LOCATIONS[i][1]), f));
         fs[i] = f;
         if (i < 6) {
      ...
      modelPartData.addChild(getLayerName(0), ModelPartBuilder.create().uv(20, 0).cuboid(-5.0F, 0.0F, (float)SEGMENT_LOCATIONS[2][2] * -0.5F, 10.0F, 8.0F, (float)SEGMENT_LOCATIONS[2][2]), ModelTransform.pivot(0.0F, 16.0F, fs[2]));
      modelPartData.addChild(getLayerName(1), ModelPartBuilder.create().uv(20, 11).cuboid(-3.0F, 0.0F, (float)SEGMENT_LOCATIONS[4][2] * -0.5F, 6.0F, 4.0F, (float)SEGMENT_LOCATIONS[4][2]), ModelTransform.pivot(0.0F, 20.0F, fs[4]));
      modelPartData.addChild(getLayerName(2), ModelPartBuilder.create().uv(20, 18).cuboid(-3.0F, 0.0F, (float)SEGMENT_LOCATIONS[4][2] * -0.5F, 6.0F, 5.0F, (float)SEGMENT_LOCATIONS[1][2]), ModelTransform.pivot(0.0F, 19.0F, fs[1]));
