...
public class WardenEntityModel extends SinglePartEntityModel {
   ...
   public void setAngles(WardenEntity wardenEntity, float f, float g, float h, float i, float j) {
   ...
   private void setLimbAngles(float angle, float distance) {
      ...
      var10000 = this.head;
      var10000.pitch += 1.2F * MathHelper.cos(g + ((float)Math.PI / 2F)) * j;
   ...
   private void setTendrilPitches(WardenEntity warden, float animationProgress, float tickDelta) {
      float f = warden.getTendrilPitch(tickDelta) * (float)(Math.cos((double)animationProgress * (double)2.25F) * Math.PI * (double)0.1F);
