...
public class BeeEntityModel extends AnimalModel {
   ...
   public void setAngles(BeeEntity beeEntity, float f, float g, float h, float i, float j) {
      ...
      if (bl) {
         ...
         this.frontLegs.pitch = ((float)Math.PI / 4F);
         this.middleLegs.pitch = ((float)Math.PI / 4F);
         this.backLegs.pitch = ((float)Math.PI / 4F);
         this.bone.pitch = 0.0F;
         this.bone.yaw = 0.0F;
         this.bone.roll = 0.0F;
      }
      ...
      if (!beeEntity.hasAngerTime()) {
         ...
         if (!bl) {
            ...
            this.frontLegs.pitch = -k * (float)Math.PI * 0.1F + ((float)Math.PI / 8F);
            this.backLegs.pitch = -k * (float)Math.PI * 0.05F + ((float)Math.PI / 4F);
