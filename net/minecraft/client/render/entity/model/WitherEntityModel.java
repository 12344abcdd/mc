...
public class WitherEntityModel extends SinglePartEntityModel {
   ...
   public void setAngles(WitherEntity witherEntity, float f, float g, float h, float i, float j) {
   ...
   public void animateModel(WitherEntity witherEntity, float f, float g, float h) {
   ...
   private static void rotateHead(WitherEntity entity, ModelPart head, int sigma) {
      head.yaw = (entity.getHeadYaw(sigma) - entity.bodyYaw) * ((float)Math.PI / 180F);
      head.pitch = entity.getHeadPitch(sigma) * ((float)Math.PI / 180F);
   }
}
