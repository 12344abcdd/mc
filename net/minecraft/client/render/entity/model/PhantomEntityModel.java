...
public class PhantomEntityModel extends SinglePartEntityModel {
   ...
   public void setAngles(PhantomEntity phantomEntity, float f, float g, float h, float i, float j) {
      float k = ((float)phantomEntity.getWingFlapTickOffset() + h) * 7.448451F * ((float)Math.PI / 180F);
      float l = 16.0F;
      this.leftWingBase.roll = MathHelper.cos(k) * 16.0F * ((float)Math.PI / 180F);
      this.leftWingTip.roll = MathHelper.cos(k) * 16.0F * ((float)Math.PI / 180F);
      this.rightWingBase.roll = -this.leftWingBase.roll;
      this.rightWingTip.roll = -this.leftWingTip.roll;
      this.tailBase.pitch = -(5.0F + MathHelper.cos(k * 2.0F) * 5.0F) * ((float)Math.PI / 180F);
      this.tailTip.pitch = -(5.0F + MathHelper.cos(k * 2.0F) * 5.0F) * ((float)Math.PI / 180F);
   }
}
