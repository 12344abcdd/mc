...
public class RavagerEntityModel extends SinglePartEntityModel {
   ...
   public static TexturedModelData getTexturedModelData() {
      ...
      modelPartData.addChild("body", ModelPartBuilder.create().uv(0, 55).cuboid(-7.0F, -10.0F, -7.0F, 14.0F, 16.0F, 20.0F).uv(0, 91).cuboid(-6.0F, 6.0F, -7.0F, 12.0F, 13.0F, 18.0F), ModelTransform.of(0.0F, 1.0F, 2.0F, ((float)Math.PI / 2F), 0.0F, 0.0F));
   ...
   public void animateModel(RavagerEntity ravagerEntity, float f, float g, float h) {
      ...
      if (l > 0) {
         ...
         this.jaw.pitch = ((float)Math.PI / 2F) * r;
         if (l > 5) {
            this.jaw.pitch = MathHelper.sin(((float)(-4 + l) - h) / 4.0F) * (float)Math.PI * 0.4F;
         } else {
            this.jaw.pitch = 0.15707964F * MathHelper.sin((float)Math.PI * ((float)l - h) / 10.0F);
         }
      } else {
         float n = -1.0F;
         ...
         if (bl) {
            ...
            this.neck.pivotX = (float)Math.sin(d * (double)10.0F) * 3.0F;
         } else if (j > 0) {
            float q = MathHelper.sin(((float)(20 - j) - h) / 20.0F * (float)Math.PI * 0.25F);
            this.jaw.pitch = ((float)Math.PI / 2F) * q;
         }
      }
