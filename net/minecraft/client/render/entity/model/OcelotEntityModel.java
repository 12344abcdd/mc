...
public class OcelotEntityModel extends AnimalModel {
   ...
   public static ModelData getModelData(Dilation dilation) {
      ...
      modelPartData.addChild("body", ModelPartBuilder.create().uv(20, 0).cuboid(-2.0F, 3.0F, -8.0F, 4.0F, 16.0F, 6.0F, dilation), ModelTransform.of(0.0F, 12.0F, -10.0F, ((float)Math.PI / 2F), 0.0F, 0.0F));
   ...
   public void setAngles(Entity entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
      ...
      if (this.animationState != 3) {
         ...
         if (this.animationState == 2) {
            ...
            this.lowerTail.pitch = 1.7278761F + ((float)Math.PI / 10F) * MathHelper.cos(limbAngle) * limbDistance;
         } else {
            this.leftHindLeg.pitch = MathHelper.cos(limbAngle * 0.6662F) * limbDistance;
            ...
            if (this.animationState == 1) {
               this.lowerTail.pitch = 1.7278761F + ((float)Math.PI / 4F) * MathHelper.cos(limbAngle) * limbDistance;
            } else {
   ...
   public void animateModel(Entity entity, float limbAngle, float limbDistance, float tickDelta) {
      ...
      if (entity.isInSneakingPose()) {
         ...
         this.upperTail.pitch = ((float)Math.PI / 2F);
         this.lowerTail.pitch = ((float)Math.PI / 2F);
         this.animationState = 0;
      } else if (entity.isSprinting()) {
         this.lowerTail.pivotY = this.upperTail.pivotY;
         ModelPart var7 = this.lowerTail;
         var7.pivotZ += 2.0F;
         this.upperTail.pitch = ((float)Math.PI / 2F);
         this.lowerTail.pitch = ((float)Math.PI / 2F);
