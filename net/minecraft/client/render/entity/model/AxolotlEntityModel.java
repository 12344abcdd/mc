...
public class AxolotlEntityModel extends AnimalModel {
   ...
   public void setAngles(AxolotlEntity axolotlEntity, float f, float g, float h, float i, float j) {
   ...
   private void resetAngles(AxolotlEntity axolotl, float headYaw, float headPitch) {
      ...
      Map<String, Vector3f> map = axolotl.getModelAngles();
      if (map.isEmpty()) {
         this.body.setAngles(headPitch * ((float)Math.PI / 180F), headYaw * ((float)Math.PI / 180F), 0.0F);
   ...
   private void setStandingOnGroundAngles(float animationProgress, float headYaw) {
      ...
      this.body.yaw = this.lerpAngleDegrees(this.body.yaw, headYaw * ((float)Math.PI / 180F));
   ...
   private void setMovingOnGroundAngles(float animationProgress, float headYaw) {
      ...
      this.setAngles(this.leftFrontLeg, 1.0995574F, ((float)Math.PI / 2F) - i, 0.0F);
      this.setAngles(this.rightHindLeg, this.leftHindLeg.pitch, -1.0F - h, 0.0F);
      this.setAngles(this.rightFrontLeg, this.leftFrontLeg.pitch, (-(float)Math.PI / 2F) - i, 0.0F);
      this.body.pitch = this.lerpAngleDegrees(0.2F, this.body.pitch, 0.0F);
      this.body.yaw = this.lerpAngleDegrees(this.body.yaw, headYaw * ((float)Math.PI / 180F));
      this.body.roll = this.lerpAngleDegrees(this.body.roll, 0.0F);
   }
   ...
   private void setStandingInWaterAngles(float animationProgress) {
      ...
      this.setAngles(this.leftFrontLeg, ((float)Math.PI / 4F) - g * 0.2F, 2.042035F, 0.0F);
      this.copyLegAngles();
   ...
   private void setMovingInWaterAngles(float animationProgress, float headPitch) {
      ...
      float h = MathHelper.cos(f);
      float i = 0.13F * g;
      this.body.pitch = this.lerpAngleDegrees(0.1F, this.body.pitch, headPitch * ((float)Math.PI / 180F) + i);
      this.head.pitch = -i * 1.8F;
      ModelPart var10000 = this.body;
      var10000.pivotY -= 0.45F * h;
      ...
      this.setAngles(this.leftHindLeg, 1.8849558F, -0.4F * g, ((float)Math.PI / 2F));
      this.setAngles(this.leftFrontLeg, 1.8849558F, -0.2F * h - 0.1F, ((float)Math.PI / 2F));
      this.copyLegAngles();
      this.head.yaw = this.lerpAngleDegrees(this.head.yaw, 0.0F);
      this.head.roll = this.lerpAngleDegrees(this.head.roll, 0.0F);
   }
   ...
   private void setPlayingDeadAngles(float headYaw) {
      this.setAngles(this.leftHindLeg, 1.4137167F, 1.0995574F, ((float)Math.PI / 4F));
      this.setAngles(this.leftFrontLeg, ((float)Math.PI / 4F), 2.042035F, 0.0F);
