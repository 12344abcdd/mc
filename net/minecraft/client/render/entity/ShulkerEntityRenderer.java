...
public class ShulkerEntityRenderer extends MobEntityRenderer {
   ...
   public boolean shouldRender(ShulkerEntity shulkerEntity, Frustum frustum, double d, double e, double f) {
      return super.shouldRender(shulkerEntity, frustum, d, e, f) ? true : shulkerEntity.getRenderPositionOffset(0.0F).filter((renderPositionOffset) -> {
         ...
         return frustum.isVisible((new Box(renderPositionOffset.x, renderPositionOffset.y + (double)f, renderPositionOffset.z, vec3d.x, vec3d.y + (double)f, vec3d.z)).expand((double)g, (double)f, (double)g));
      }).isPresent();
   }
   ...
   static {
      TEXTURE = TexturedRenderLayers.SHULKER_TEXTURE_ID.getTextureId().withPath((string) -> "textures/" + string + ".png");
      COLORED_TEXTURES = (Identifier[])TexturedRenderLayers.COLORED_SHULKER_BOXES_TEXTURES.stream().map((spriteId) -> spriteId.getTextureId().withPath((string) -> "textures/" + string + ".png")).toArray((i) -> new Identifier[i]);
   }
}
