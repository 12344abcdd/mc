...
public class ShoulderParrotFeatureRenderer extends FeatureRenderer {
   ...
   public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, PlayerEntity playerEntity, float f, float g, float h, float j, float k, float l) {
   ...
   private void renderShoulderParrot(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, PlayerEntity player, float limbAngle, float limbDistance, float headYaw, float headPitch, boolean leftShoulder) {
      ...
      EntityType.get(nbtCompound.getString("id")).filter((type) -> type == EntityType.PARROT).ifPresent((type) -> {
         ...
         ParrotEntity.Variant variant = Variant.byIndex(nbtCompound.getInt("Variant"));
