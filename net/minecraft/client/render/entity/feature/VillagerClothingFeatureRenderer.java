...
public class VillagerClothingFeatureRenderer extends FeatureRenderer {
   ...
   public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, LivingEntity livingEntity, float f, float g, float h, float j, float k, float l) {
      if (!livingEntity.isInvisible()) {
         ...
         M entityModel = (M)this.getContextModel();
   ...
   public VillagerResourceMetadata.HatType getHatType(Object2ObjectMap hatLookUp, String keyType, DefaultedRegistry registry, Object key) {
      return (VillagerResourceMetadata.HatType)hatLookUp.computeIfAbsent(key, (k) -> (VillagerResourceMetadata.HatType)this.resourceManager.getResource(this.findTexture(keyType, registry.getId(key))).flatMap((resource) -> {
            try {
