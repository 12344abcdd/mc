...
public class StuckStingersFeatureRenderer extends StuckObjectsFeatureRenderer {
   ...
   public StuckStingersFeatureRenderer(LivingEntityRenderer livingEntityRenderer) {
   ...
   protected void renderObject(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, Entity entity, float directionX, float directionY, float directionZ, float tickDelta) {
      ...
      float g = (float)(Math.atan2((double)directionX, (double)directionZ) * (double)(180F / (float)Math.PI));
      float h = (float)(Math.atan2((double)directionY, (double)f) * (double)(180F / (float)Math.PI));
   ...
   private static void produceVertex(VertexConsumer vertexConsumer, MatrixStack.Entry matrix, float x, int y, float u, float v, int light) {
      vertexConsumer.vertex(matrix, x, (float)y, 0.0F).color(-1).texture(u, v).overlay(OverlayTexture.DEFAULT_UV).light(light).normal(matrix, 0.0F, 1.0F, 0.0F);
   }
}
