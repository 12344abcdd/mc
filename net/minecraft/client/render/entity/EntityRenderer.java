...
public abstract class EntityRenderer {
   ...
   public boolean shouldRender(Entity entity, Frustum frustum, double x, double y, double z) {
      if (!entity.shouldRender(x, y, z)) {
         ...
         Box box = entity.getVisibilityBoundingBox().expand((double)0.5F);
         if (box.isNaN() || box.getAverageSideLength() == (double)0.0F) {
            box = new Box(entity.getX() - (double)2.0F, entity.getY() - (double)2.0F, entity.getZ() - (double)2.0F, entity.getX() + (double)2.0F, entity.getY() + (double)2.0F, entity.getZ() + (double)2.0F);
         }
   ...
   private void renderLeash(Entity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, Entity leashHolder) {
      matrices.push();
      Vec3d vec3d = leashHolder.getLeashPos(tickDelta);
      double d = (double)(entity.lerpYaw(tickDelta) * ((float)Math.PI / 180F)) + (Math.PI / 2D);
   ...
   protected void renderLabelIfPresent(Entity entity, Text text, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, float tickDelta) {
      ...
      if (!(d > (double)4096.0F)) {
         ...
         if (vec3d != null) {
            ...
            float g = (float)(-textRenderer.getWidth((StringVisitable)text) / 2);
            ...
            if (bl) {
               textRenderer.draw((Text)text, g, (float)i, -1, false, matrix4f, vertexConsumers, TextRenderer.TextLayerType.NORMAL, 0, light);
            }
