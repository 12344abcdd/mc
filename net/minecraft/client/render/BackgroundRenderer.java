...
public class BackgroundRenderer {
   ...
   public static void render(Camera camera, float tickDelta, ClientWorld world, int viewDistance, float skyDarkness) {
      ...
      if (cameraSubmersionType == CameraSubmersionType.WATER) {
         ...
         r = 1.0F - (float)Math.pow((double)r, (double)0.25F);
         Vec3d vec3d = world.getSkyColor(camera.getPos(), tickDelta);
         float s = (float)vec3d.x;
         float t = (float)vec3d.y;
         float u = (float)vec3d.z;
         float v = MathHelper.clamp(MathHelper.cos(world.getSkyAngle(tickDelta) * ((float)Math.PI * 2F)) * 2.0F + 0.5F, 0.0F, 1.0F);
         BiomeAccess biomeAccess = world.getBiomeAccess();
         Vec3d vec3d2 = camera.getPos().subtract((double)2.0F, (double)2.0F, (double)2.0F).multiply((double)0.25F);
         Vec3d vec3d3 = CubicSampler.sampleColor(vec3d2, (x, y, z) -> world.getDimensionEffects().adjustFogColor(Vec3d.unpackRgb(((Biome)biomeAccess.getBiomeForNoiseGen(x, y, z).value()).getFogColor()), v));
   ...
   private static StatusEffectFogModifier getFogModifier(Entity entity, float tickDelta) {
      if (entity instanceof LivingEntity livingEntity) {
         return (StatusEffectFogModifier)FOG_MODIFIERS.stream().filter((modifier) -> modifier.shouldApply(livingEntity, tickDelta)).findFirst().orElse((Object)null);
      } else {
         return null;
      }
   }
