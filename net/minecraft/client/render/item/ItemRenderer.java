...
public class ItemRenderer implements SynchronousResourceReloader {
   ...
   public static VertexConsumer getDirectItemGlintConsumer(VertexConsumerProvider provider, RenderLayer layer, boolean solid, boolean glint) {
      return glint ? VertexConsumers.union(provider.getBuffer(solid ? RenderLayer.getGlint() : RenderLayer.getDirectEntityGlint()), provider.getBuffer(layer)) : provider.getBuffer(layer);
   }
   ...
   private void renderBakedItemQuads(MatrixStack matrices, VertexConsumer vertices, List quads, ItemStack stack, int light, int overlay) {
      ...
      for(BakedQuad bakedQuad : quads) {
         ...
         float f = (float)Argb.getAlpha(i) / 255.0F;
   ...
   static {
      ...
      TRIDENT = ModelIdentifier.ofInventoryVariant(Identifier.ofVanilla("trident"));
      TRIDENT_IN_HAND = ModelIdentifier.ofInventoryVariant(Identifier.ofVanilla("trident_in_hand"));
      SPYGLASS = ModelIdentifier.ofInventoryVariant(Identifier.ofVanilla("spyglass"));
      SPYGLASS_IN_HAND = ModelIdentifier.ofInventoryVariant(Identifier.ofVanilla("spyglass_in_hand"));
   }
}
