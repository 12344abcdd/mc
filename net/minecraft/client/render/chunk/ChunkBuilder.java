...
public class ChunkBuilder {
   ...
   private void scheduleRunTasks() {
      if (!this.stopped && !this.buffersPool.hasNoAvailableBuilder()) {
         ...
         if (task != null) {
            ...
            CompletableFuture.supplyAsync(Util.debugSupplier(task.getName(), () -> task.run(blockBufferAllocatorStorage)), this.executor).thenCompose((future) -> future).whenComplete((result, throwable) -> {
               if (throwable != null) {
   ...
   private BuiltChunk.Task pollTask() {
      if (this.processablePrioritizedTaskCount <= 0) {
         BuiltChunk.Task task = (BuiltChunk.Task)this.taskQueue.poll();
         if (task != null) {
      ...
      BuiltChunk.Task task = (BuiltChunk.Task)this.prioritizedTaskQueue.poll();
      if (task != null) {
   ...
   public CompletableFuture scheduleUpload(BuiltBuffer builtBuffer, VertexBuffer glBuffer) {
      if (this.stopped) {
   ...
   public CompletableFuture method_60906(BufferAllocator.CloseableBuffer closeableBuffer, VertexBuffer vertexBuffer) {
      if (this.stopped) {
         return CompletableFuture.completedFuture((Object)null);
      } else {
         Runnable var10000 = () -> {
   ...
   public class BuiltChunk {
      ...
      public BuiltChunk(final int index, final int originX, final int originY, final int originZ) {
         ...
         this.buffers = (Map)RenderLayer.getBlockLayers().stream().collect(Collectors.toMap((layer) -> layer, (layer) -> new VertexBuffer(VertexBuffer.Usage.STATIC)));
      ...
      public boolean shouldBuild() {
         int i = 24;
         if (!(this.getSquaredCameraDistance() > (double)576.0F)) {
            return true;
         } else {
      ...
      protected double getSquaredCameraDistance() {
         ...
         double d = this.boundingBox.minX + (double)8.0F - camera.getPos().x;
         double e = this.boundingBox.minY + (double)8.0F - camera.getPos().y;
         double f = this.boundingBox.minZ + (double)8.0F - camera.getPos().z;
         return d * d + e * e + f * f;
      }
      ...
      void setNoCullingBlockEntities(Collection blockEntities) {
      ...
      class SortTask extends Task {
         ...
         public SortTask(final double distance, final ChunkData data) {
            super(distance, true);
            this.data = data;
         }
            ...
            if (this.cancelled.get()) {
               ...
               if (sortState != null && !this.data.isEmpty(RenderLayer.getTranslucent())) {
                  ...
                  if (closeableBuffer == null) {
                     ...
                     CompletableFuture<Result> completableFuture = ChunkBuilder.this.method_60906(closeableBuffer, BuiltChunk.this.getBuffer(RenderLayer.getTranslucent())).thenApply((v) -> ChunkBuilder.Result.CANCELLED);
