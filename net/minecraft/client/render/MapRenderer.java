...
public class MapRenderer implements AutoCloseable {
   ...
   private MapTexture getMapTexture(MapIdComponent id, MapState state) {
      return (MapTexture)this.mapTextures.compute(id.id(), (id2, texture) -> {
         if (texture == null) {
      ...
      void draw(MatrixStack matrices, VertexConsumerProvider vertexConsumers, boolean hidePlayerIcons, int light) {
         ...
         for(MapDecoration mapDecoration : this.state.getDecorations()) {
            if (!hidePlayerIcons || mapDecoration.isAlwaysRendered()) {
               ...
               vertexConsumer2.vertex(matrix4f2, -1.0F, 1.0F, (float)k * -0.001F).color(-1).texture(h, l).light(light);
               vertexConsumer2.vertex(matrix4f2, 1.0F, 1.0F, (float)k * -0.001F).color(-1).texture(m, l).light(light);
               vertexConsumer2.vertex(matrix4f2, 1.0F, -1.0F, (float)k * -0.001F).color(-1).texture(m, n).light(light);
               vertexConsumer2.vertex(matrix4f2, -1.0F, -1.0F, (float)k * -0.001F).color(-1).texture(h, n).light(light);
               matrices.pop();
               if (mapDecoration.name().isPresent()) {
                  ...
                  textRenderer.draw((Text)text, 0.0F, 0.0F, -1, false, matrices.peek().getPositionMatrix(), vertexConsumers, TextRenderer.TextLayerType.NORMAL, Integer.MIN_VALUE, light);
                  matrices.pop();
               }
