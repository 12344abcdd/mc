...
public class PathfindingDebugRenderer implements DebugRenderer.Renderer {
   ...
   public static void drawPath(MatrixStack matrices, VertexConsumerProvider vertexConsumers, Path path, float nodeSize, boolean drawDebugNodes, boolean drawLabels, double cameraX, double cameraY, double cameraZ) {
      drawPathLines(matrices, vertexConsumers.getBuffer(RenderLayer.getDebugLineStrip((double)6.0F)), path, cameraX, cameraY, cameraZ);
      BlockPos blockPos = path.getTarget();
      if (getManhattanDistance(blockPos, cameraX, cameraY, cameraZ) <= 80.0F) {
         DebugRenderer.drawBox(matrices, vertexConsumers, (new Box((double)((float)blockPos.getX() + 0.25F), (double)((float)blockPos.getY() + 0.25F), (double)blockPos.getZ() + (double)0.25F, (double)((float)blockPos.getX() + 0.75F), (double)((float)blockPos.getY() + 0.75F), (double)((float)blockPos.getZ() + 0.75F))).offset(-cameraX, -cameraY, -cameraZ), 0.0F, 1.0F, 0.0F, 0.5F);
      ...
      if (drawLabels) {
         for(int j = 0; j < path.getLength(); ++j) {
            ...
            if (getManhattanDistance(pathNode3.getBlockPos(), cameraX, cameraY, cameraZ) <= 80.0F) {
               DebugRenderer.drawString(matrices, vertexConsumers, String.valueOf(pathNode3.type), (double)pathNode3.x + (double)0.5F, (double)pathNode3.y + (double)0.75F, (double)pathNode3.z + (double)0.5F, -1, 0.02F, true, 0.0F, true);
               DebugRenderer.drawString(matrices, vertexConsumers, String.format(Locale.ROOT, "%.2f", pathNode3.penalty), (double)pathNode3.x + (double)0.5F, (double)pathNode3.y + (double)0.25F, (double)pathNode3.z + (double)0.5F, -1, 0.02F, true, 0.0F, true);
            }
         }
      }
   ...
   public static void drawPathLines(MatrixStack matrices, VertexConsumer vertexConsumers, Path path, double cameraX, double cameraY, double cameraZ) {
      for(int i = 0; i < path.getLength(); ++i) {
         ...
         if (!(getManhattanDistance(pathNode.getBlockPos(), cameraX, cameraY, cameraZ) > 80.0F)) {
            ...
            int l = j >> 8 & 255;
            int m = j & 255;
            vertexConsumers.vertex(matrices.peek(), (float)((double)pathNode.x - cameraX + (double)0.5F), (float)((double)pathNode.y - cameraY + (double)0.5F), (float)((double)pathNode.z - cameraZ + (double)0.5F)).color(k, l, m, 255);
         }
      }
