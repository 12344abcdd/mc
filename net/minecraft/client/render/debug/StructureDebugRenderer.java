...
public class StructureDebugRenderer implements DebugRenderer.Renderer {
   ...
   public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
      ...
      if (this.structureBoundingBoxes.containsKey(registryKey)) {
         for(BlockBox blockBox : ((Map)this.structureBoundingBoxes.get(registryKey)).values()) {
            if (blockPos.isWithinDistance(blockBox.getCenter(), (double)500.0F)) {
      ...
      Map<String, DebugStructuresCustomPayload.Piece> map = (Map)this.structurePiecesBoundingBoxes.get(registryKey);
      if (map != null) {
         for(DebugStructuresCustomPayload.Piece piece : map.values()) {
            BlockBox blockBox2 = piece.boundingBox();
            if (blockPos.isWithinDistance(blockBox2.getCenter(), (double)500.0F)) {
               if (piece.isStart()) {
   ...
   public void addStructure(BlockBox boundingBox, List pieces, RegistryKey dimensionKey) {
      ...
      Map<String, DebugStructuresCustomPayload.Piece> map = (Map)this.structurePiecesBoundingBoxes.computeIfAbsent(dimensionKey, (dimension) -> new HashMap());
