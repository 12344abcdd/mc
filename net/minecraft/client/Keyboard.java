...
public class Keyboard {
   ...
   private void addDebugMessage(Formatting formatting, Text text) {
      this.client.inGameHud.getChatHud().addMessage((Text)Text.empty().append(Text.translatable("debug.prefix").formatted(new Formatting[]{formatting, Formatting.BOLD})).append(ScreenTexts.SPACE).append(text));
   }
      ...
      if (this.debugCrashStartTime > 0L && this.debugCrashStartTime < Util.getMeasuringTimeMs() - 100L) {
         ...
         switch (key) {
            ...
            case 81:
               ...
               chatHud.addMessage((Text)Text.translatable("debug.reload_chunks.help"));
               chatHud.addMessage((Text)Text.translatable("debug.show_hitboxes.help"));
               chatHud.addMessage((Text)Text.translatable("debug.copy_location.help"));
               chatHud.addMessage((Text)Text.translatable("debug.clear_chat.help"));
               chatHud.addMessage((Text)Text.translatable("debug.chunk_boundaries.help"));
               chatHud.addMessage((Text)Text.translatable("debug.advanced_tooltips.help"));
               chatHud.addMessage((Text)Text.translatable("debug.inspect.help"));
               chatHud.addMessage((Text)Text.translatable("debug.profiling.help"));
               chatHud.addMessage((Text)Text.translatable("debug.creative_spectator.help"));
               chatHud.addMessage((Text)Text.translatable("debug.pause_focus.help"));
               chatHud.addMessage((Text)Text.translatable("debug.help.help"));
               chatHud.addMessage((Text)Text.translatable("debug.dump_dynamic_textures.help"));
               chatHud.addMessage((Text)Text.translatable("debug.reload_resourcepacks.help"));
               chatHud.addMessage((Text)Text.translatable("debug.pause.help"));
               chatHud.addMessage((Text)Text.translatable("debug.gamemodes.help"));
               return true;
            case 83:
               ...
               Text text = Text.literal(path.relativize(path2).toString()).formatted(Formatting.UNDERLINE).styled((style) -> style.withClickEvent(new ClickEvent(Action.OPEN_FILE, path2.toFile().toString())));
      ...
      if (window == this.client.getWindow().getHandle()) {
         ...
         if (action == 1 && (!(this.client.currentScreen instanceof KeybindsScreen) || ((KeybindsScreen)screen).lastKeyCodeUpdateTime <= Util.getMeasuringTimeMs() - 20L)) {
            ...
            if (this.client.options.screenshotKey.matchesKey(key, scancode)) {
               ...
               ScreenshotRecorder.saveScreenshot(this.client.runDirectory, this.client.getFramebuffer(), (message) -> this.client.execute(() -> this.client.inGameHud.getChatHud().addMessage(message)));
               return;
            }
         }
         ...
         if (action != 0) {
            ...
            if (bl2) {
               ...
               ClientPlayerEntity bl3 = this.client.player;
            }
         }
   ...
   public void setup(long window) {
      InputUtil.setKeyboardCallbacks(window, (windowx, key, scancode, action, modifiers) -> this.client.execute(() -> this.onKey(windowx, key, scancode, action, modifiers)), (windowx, codePoint, modifiers) -> this.client.execute(() -> this.onChar(windowx, codePoint, modifiers)));
   }
