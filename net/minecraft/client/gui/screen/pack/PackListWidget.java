...
public class PackListWidget extends AlwaysSelectedEntryListWidget {
   ...
   protected void renderHeader(DrawContext context, int x, int y) {
      ...
      context.drawText(this.client.textRenderer, (Text)text, x + this.width / 2 - this.client.textRenderer.getWidth((StringVisitable)text) / 2, Math.min(this.getY() + 3, y), -1, false);
   }
   ...
   public static class ResourcePackEntry extends AlwaysSelectedEntryListWidget.Entry {
      ...
      private static OrderedText trimTextToWidth(MinecraftClient client, Text text) {
         ...
         if (i > 157) {
            StringVisitable stringVisitable = StringVisitable.concat(new StringVisitable[]{client.textRenderer.trimToWidth((StringVisitable)text, 157 - client.textRenderer.getWidth("...")), StringVisitable.plain("...")});
      ...
      public boolean mouseClicked(double mouseX, double mouseY, int button) {
         ...
         if (this.isSelectable() && d <= (double)32.0F) {
            ...
            if (d < (double)16.0F && this.pack.canBeDisabled()) {
               this.pack.disable();
               return true;
            }

            if (d > (double)16.0F && e < (double)16.0F && this.pack.canMoveTowardStart()) {
               this.pack.moveTowardStart();
               return true;
            }

            if (d > (double)16.0F && e > (double)16.0F && this.pack.canMoveTowardEnd()) {
               this.pack.moveTowardEnd();
               return true;
            }
         }
