      ...
      Thread thread = new Thread("Server Connector #" + CONNECTOR_THREADS_COUNT.incrementAndGet()) {
         public void run() {
            ...
            try {
               ...
               if (optional.isEmpty()) {
                  client.execute(() -> client.setScreen(new DisconnectedScreen(ConnectScreen.this.parent, ConnectScreen.this.failureErrorMessage, ConnectScreen.UNKNOWN_HOST_TEXT)));
                  return;
               }
               ...
               client.execute(() -> client.setScreen(new DisconnectedScreen(ConnectScreen.this.parent, ConnectScreen.this.failureErrorMessage, Text.translatable("disconnect.genericReason", new Object[]{string}))));
            }
         ...
         private static ServerResourcePackManager.AcceptanceStatus toAcceptanceStatus(ServerInfo.ResourcePackPolicy policy) {
            ...
            switch (policy) {
               case ENABLED -> var10000 = ServerResourcePackManager.AcceptanceStatus.ALLOWED;
               case DISABLED -> var10000 = ServerResourcePackManager.AcceptanceStatus.DECLINED;
               case PROMPT -> var10000 = ServerResourcePackManager.AcceptanceStatus.PENDING;
