...
public class AddServerScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addDrawableChild(CyclingButtonWidget.builder(ServerInfo.ResourcePackPolicy::getName).values((Object[])ServerInfo.ResourcePackPolicy.values()).initially(this.server.getResourcePackPolicy()).build(this.width / 2 - 100, this.height / 4 + 72, 200, 20, Text.translatable("addServer.resourcePack"), (button, resourcePackPolicy) -> this.server.setResourcePackPolicy(resourcePackPolicy)));
      this.addButton = (ButtonWidget)this.addDrawableChild(ButtonWidget.builder(Text.translatable("addServer.add"), (button) -> this.addAndClose()).dimensions(this.width / 2 - 100, this.height / 4 + 96 + 18, 200, 20).build());
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawTextWithShadow(this.textRenderer, (Text)ENTER_NAME_TEXT, this.width / 2 - 100 + 1, 53, 10526880);
      context.drawTextWithShadow(this.textRenderer, (Text)ENTER_IP_TEXT, this.width / 2 - 100 + 1, 94, 10526880);
