...
public class ScreenNarrator {
   ...
   final Map narrations = Maps.newTreeMap(Comparator.comparing((partIndex) -> partIndex.part).thenComparing((partIndex) -> partIndex.depth));
   ...
   class MessageBuilder implements NarrationMessageBuilder {
      ...
      public void put(NarrationPart part, Narration narration) {
         ((Message)ScreenNarrator.this.narrations.computeIfAbsent(new PartIndex(part, this.depth), (partIndex) -> new Message())).setNarration(ScreenNarrator.this.currentMessageIndex, narration);
      }
