...
public class SoundOptionsScreen extends GameOptionsScreen {
   ...
   private SimpleOption[] getVolumeOptions() {
      return (SimpleOption[])Arrays.stream(SoundCategory.values()).filter((category) -> category != SoundCategory.MASTER).map((category) -> this.gameOptions.getSoundVolumeOption(category)).toArray((i) -> new SimpleOption[i]);
   }
}
