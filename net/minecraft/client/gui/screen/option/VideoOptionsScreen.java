...
public class VideoOptionsScreen extends GameOptionsScreen {
   ...
   protected void addOptions() {
      ...
      SimpleOption<Integer> simpleOption = new SimpleOption("options.fullscreen.resolution", SimpleOption.emptyTooltip(), (optionText, value) -> {
         if (monitor == null) {
            ...
            return GameOptions.getGenericValueText(optionText, Text.translatable("options.fullscreen.entry", new Object[]{videoMode.getWidth(), videoMode.getHeight(), videoMode.getRefreshRate(), videoMode.getRedBits() + videoMode.getGreenBits() + videoMode.getBlueBits()}));
         }
      ...
      if (Screen.hasControlDown()) {
         ...
         if (var11 instanceof SimpleOption.MaxSuppliableIntCallbacks) {
            ...
            if (k != 0 && k <= maxSuppliableIntCallbacks.maxInclusive() && k >= maxSuppliableIntCallbacks.minInclusive()) {
               CyclingButtonWidget<Integer> cyclingButtonWidget = (CyclingButtonWidget)this.body.getWidgetFor(simpleOption);
               if (cyclingButtonWidget != null) {
                  simpleOption.setValue(k);
                  cyclingButtonWidget.setValue(k);
                  this.body.setScrollAmount((double)0.0F);
                  return true;
               }
            }
         }
   ...
   static {
      ...
      GRAPHICS_WARNING_MESSAGE_TEXT = Text.translatable("options.graphics.warning.message", new Object[]{GRAPHICS_FABULOUS_TEXT, GRAPHICS_FABULOUS_TEXT});
