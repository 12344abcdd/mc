...
public class OnlineOptionsScreen extends GameOptionsScreen {
   ...
   private SimpleOption[] collectOptions(GameOptions gameOptions, MinecraftClient client) {
      ...
      SimpleOption<Unit> simpleOption = (SimpleOption)Nullables.map(client.world, (world) -> {
         Difficulty difficulty = world.getDifficulty();
         return new SimpleOption("options.difficulty.online", SimpleOption.emptyTooltip(), (text, unit) -> difficulty.getTranslatableName(), new SimpleOption.PotentialValuesBasedCallbacks(List.of(Unit.INSTANCE), Codec.EMPTY.codec()), Unit.INSTANCE, (unit) -> {
         });
      });
