...
public class LoomScreen extends HandledScreen {
   ...
   protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
      ...
      if (this.bannerPatterns != null && !this.hasTooManyPatterns) {
         ...
         BannerBlockEntityRenderer.renderCanvas(context.getMatrices(), context.getVertexConsumers(), 15728880, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, dyeColor, this.bannerPatterns);
      ...
      if (this.canApplyDyePattern) {
         ...
         for(int n = 0; n < 4; ++n) {
            for(int o = 0; o < 4; ++o) {
               ...
               this.drawBanner(context, (RegistryEntry)list.get(q), r, s);
            }
         }
      }
      ...
      if (this.canApplyDyePattern) {
         ...
         for(int k = 0; k < 4; ++k) {
            for(int l = 0; l < 4; ++l) {
               ...
               int n = m * 4 + l;
               if (d >= (double)0.0F && e >= (double)0.0F && d < (double)14.0F && e < (double)14.0F && ((LoomScreenHandler)this.handler).onButtonClick(this.client.player, n)) {
