...
public class StructureBlockScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addDrawableChild(CyclingButtonWidget.builder((value) -> Text.translatable("structure_block.mode." + value.asString())).values(MODES_EXCEPT_DATA, MODES).omitKeyText().initially(this.mode).build(this.width / 2 - 4 - 150, 185, 50, 20, Text.literal("MODE"), (button, mode) -> {
      ...
      this.buttonEntities = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(!this.structureBlock.shouldIgnoreEntities()).omitKeyText().build(this.width / 2 + 4 + 100, 160, 50, 20, INCLUDE_ENTITIES_TEXT, (button, includeEntities) -> this.structureBlock.setIgnoreEntities(!includeEntities)));
      this.buttonMirror = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.builder(BlockMirror::getName).values((Object[])BlockMirror.values()).omitKeyText().initially(this.mirror).build(this.width / 2 - 20, 185, 40, 20, Text.literal("MIRROR"), (button, mirror) -> this.structureBlock.setMirror(mirror)));
      this.buttonShowAir = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowAir()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_AIR_TEXT, (button, showAir) -> this.structureBlock.setShowAir(showAir)));
      this.buttonShowBoundingBox = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowBoundingBox()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_BOUNDING_BOX_TEXT, (button, showBoundingBox) -> this.structureBlock.setShowBoundingBox(showBoundingBox)));
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawTextWithShadow(this.textRenderer, (Text)structureBlockMode.asText(), this.width / 2 - 153, 174, 10526880);
   }
   ...
   static {
      MODES_EXCEPT_DATA = (ImmutableList)MODES.stream().filter((mode) -> mode != StructureBlockMode.DATA).collect(ImmutableList.toImmutableList());
   }
}
