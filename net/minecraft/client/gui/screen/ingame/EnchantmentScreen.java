...
public class EnchantmentScreen extends HandledScreen {
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      ...
      for(int k = 0; k < 3; ++k) {
         ...
         if (d >= (double)0.0F && e >= (double)0.0F && d < (double)108.0F && e < (double)19.0F && ((EnchantmentScreenHandler)this.handler).onButtonClick(this.client.player, k)) {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      for(int j = 0; j < 3; ++j) {
         ...
         Optional<RegistryEntry.Reference<Enchantment>> optional = this.client.world.getRegistryManager().get(RegistryKeys.ENCHANTMENT).getEntry(((EnchantmentScreenHandler)this.handler).enchantmentId[j]);
         if (!optional.isEmpty()) {
            ...
            if (this.isPointWithinBounds(60, 14 + 19 * j, 108, 17, (double)mouseX, (double)mouseY) && k > 0 && l >= 0 && optional != null) {
               ...
               if (!bl) {
                  ...
                  if (this.client.player.experienceLevel < k) {
                     list.add(Text.translatable("container.enchant.level.requirement", new Object[]{((EnchantmentScreenHandler)this.handler).enchantmentPower[j]}).formatted(Formatting.RED));
                  } else {
