...
public class CommandBlockScreen extends AbstractCommandBlockScreen {
   ...
   protected void init() {
      ...
      this.modeButton = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.builder((value) -> {
         ...
         switch (value) {
            case SEQUENCE -> var10000 = Text.translatable("advMode.mode.sequence");
            case AUTO -> var10000 = Text.translatable("advMode.mode.auto");
            case REDSTONE -> var10000 = Text.translatable("advMode.mode.redstone");
            default -> throw new MatchException((String)null, (Throwable)null);
         }

         return var10000;
      }).values((Object[])Type.values()).omitKeyText().initially(this.mode).build(this.width / 2 - 50 - 100 - 4, 165, 100, 20, Text.translatable("advMode.mode"), (button, mode) -> this.mode = mode));
      ...
      this.redstoneTriggerButton = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(Text.translatable("advMode.mode.autoexec.bat"), Text.translatable("advMode.mode.redstoneTriggered")).omitKeyText().initially(this.autoActivate).build(this.width / 2 + 50 + 4, 165, 100, 20, Text.translatable("advMode.triggering"), (button, autoActivate) -> this.autoActivate = autoActivate));
