...
public class JigsawBlockScreen extends Screen {
   ...
   protected void init() {
      ...
      this.jointRotationButton = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.builder(JigsawBlockEntity.Joint::asText).values((Object[])Joint.values()).initially(this.joint).omitKeyText().build(this.width / 2 + 54, 160, 100, 20, JOINT_LABEL_TEXT, (button, joint) -> this.joint = joint));
      ...
      this.addDrawableChild(new SliderWidget(this.width / 2 - 154, 185, 100, 20, ScreenTexts.EMPTY, (double)0.0F) {
         {
            this.updateMessage();
         }
         ...
         protected void applyValue() {
            JigsawBlockScreen.this.generationDepth = MathHelper.floor(MathHelper.clampedLerp((double)0.0F, (double)20.0F, this.value));
         }
      });
      this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.keepJigsaws).build(this.width / 2 - 50, 185, 100, 20, Text.translatable("jigsaw_block.keep_jigsaws"), (button, keepJigsaws) -> this.keepJigsaws = keepJigsaws));
      ...
      this.doneButton = (ButtonWidget)this.addDrawableChild(ButtonWidget.builder(ScreenTexts.DONE, (button) -> this.onDone()).dimensions(this.width / 2 - 4 - 150, 210, 150, 20).build());
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawTextWithShadow(this.textRenderer, (Text)SELECTION_PRIORITY_TEXT, this.width / 2 - 153, 150, 10526880);
      this.placementPriorityField.render(context, mouseX, mouseY, delta);
      context.drawTextWithShadow(this.textRenderer, (Text)PLACEMENT_PRIORITY_TEXT, this.width / 2 - 50, 150, 10526880);
      ...
      if (JigsawBlock.getFacing(this.jigsaw.getCachedState()).getAxis().isVertical()) {
         context.drawTextWithShadow(this.textRenderer, (Text)JOINT_LABEL_TEXT, this.width / 2 + 53, 150, 10526880);
      }
