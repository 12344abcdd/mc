...
public abstract class AbstractSignEditScreen extends Screen {
   ...
   public AbstractSignEditScreen(SignBlockEntity blockEntity, boolean front, boolean filtered, Text title) {
      ...
      this.messages = (String[])IntStream.range(0, 4).mapToObj((line) -> this.text.getMessage(line, filtered)).map(Text::getString).toArray((i) -> new String[i]);
   }
   ...
   protected void init() {
      ...
      this.selectionManager = new SelectionManager(() -> this.messages[this.currentRow], this::setCurrentRowMessage, SelectionManager.makeClipboardGetter(this.client), SelectionManager.makeClipboardSetter(this.client), (string) -> this.client.textRenderer.getWidth(string) <= this.blockEntity.getMaxTextWidth());
   }
