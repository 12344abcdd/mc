...
public interface LevelScreenProvider {
   Map WORLD_PRESET_TO_SCREEN_PROVIDER = Map.of(Optional.of(WorldPresets.FLAT), (LevelScreenProvider)(parent, generatorOptionsHolder) -> {
      ...
      return new CustomizeFlatLevelScreen(parent, (config) -> parent.getWorldCreator().applyModifier(createModifier(config)), chunkGenerator instanceof FlatChunkGenerator ? ((FlatChunkGenerator)chunkGenerator).getConfig() : FlatChunkGeneratorConfig.getDefaultConfig(registryEntryLookup, registryEntryLookup2, registryEntryLookup3));
   }, Optional.of(WorldPresets.SINGLE_BIOME_SURFACE), (LevelScreenProvider)(parent, generatorOptionsHolder) -> new CustomizeBuffetLevelScreen(parent, generatorOptionsHolder, (biomeEntry) -> parent.getWorldCreator().applyModifier(createModifier(biomeEntry))));
