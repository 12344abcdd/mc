...
public class CustomizeBuffetLevelScreen extends Screen {
   ...
   public CustomizeBuffetLevelScreen(Screen parent, GeneratorOptionsHolder generatorOptionsHolder, Consumer onDone) {
      ...
      RegistryEntry<Biome> registryEntry = (RegistryEntry)this.biomeRegistry.getEntry(BiomeKeys.PLAINS).or(() -> this.biomeRegistry.streamEntries().findAny()).orElseThrow();
   ...
   protected void init() {
      ...
      this.biomeSelectionList.setSelected((BuffetBiomesListWidget.BuffetBiomeItem)this.biomeSelectionList.children().stream().filter((entry) -> Objects.equals(entry.biome, this.biome)).findFirst().orElse((Object)null));
   ...
   class BuffetBiomesListWidget extends AlwaysSelectedEntryListWidget {
      BuffetBiomesListWidget() {
         ...
         CustomizeBuffetLevelScreen.this.biomeRegistry.streamEntries().map((entry) -> new BuffetBiomeItem(entry)).sorted(Comparator.comparing((biome) -> biome.text.getString(), collator)).forEach((entry) -> this.addEntry(entry));
      }
      ...
      @Environment(EnvType.CLIENT)
      class BuffetBiomeItem extends AlwaysSelectedEntryListWidget.Entry {
