...
public class WorldCreator {
   ...
   public void update() {
      ...
      if (bl != this.generatorOptionsHolder.generatorOptions().hasBonusChest()) {
         this.generatorOptionsHolder = this.generatorOptionsHolder.apply((GeneratorOptionsHolder.Modifier)((options) -> options.withBonusChest(bl)));
      }

      boolean bl2 = this.shouldGenerateStructures();
      if (bl2 != this.generatorOptionsHolder.generatorOptions().shouldGenerateStructures()) {
         this.generatorOptionsHolder = this.generatorOptionsHolder.apply((GeneratorOptionsHolder.Modifier)((options) -> options.withStructures(bl2)));
      }
   ...
   public void setSeed(String seed) {
      this.seed = seed;
      this.generatorOptionsHolder = this.generatorOptionsHolder.apply((GeneratorOptionsHolder.Modifier)((options) -> options.withSeed(GeneratorOptions.parseSeed(this.getSeed()))));
      this.update();
   }
   ...
   private void updateWorldTypeLists() {
      ...
      this.normalWorldTypes.addAll((Collection)getWorldPresetList(registry, WorldPresetTags.NORMAL).orElseGet(() -> registry.streamEntries().map(WorldType::new).toList()));
      ...
      if (registryEntry != null) {
         this.worldType = (WorldType)getWorldPreset(this.getGeneratorOptionsHolder(), registryEntry.getKey()).map(WorldType::new).orElse((WorldType)this.normalWorldTypes.get(0));
      }
   ...
   public static record WorldType(@Nullable RegistryEntry preset) {
      ...
      public Text getName() {
         return (Text)Optional.ofNullable(this.preset).flatMap(RegistryEntry::getKey).map((key) -> Text.translatable(key.getValue().toTranslationKey("generator"))).orElse(CUSTOM_GENERATOR_TEXT);
      }
