...
public class CreateWorldScreen extends Screen {
   ...
   public static CreateWorldScreen create(MinecraftClient client, @Nullable Screen parent, LevelInfo levelInfo, GeneratorOptionsHolder generatorOptionsHolder, @Nullable Path dataPackTempDir) {
      CreateWorldScreen createWorldScreen = new CreateWorldScreen(client, parent, generatorOptionsHolder, WorldPresets.getWorldPreset(generatorOptionsHolder.selectedDimensions()), OptionalLong.of(generatorOptionsHolder.generatorOptions().getSeed()));
   ...
   private void createLevel() {
      ...
      CombinedDynamicRegistries<ServerDynamicRegistryType> combinedDynamicRegistries = generatorOptionsHolder.combinedDynamicRegistries().with(ServerDynamicRegistryType.DIMENSIONS, new DynamicRegistryManager.Immutable[]{dimensionsConfig.toDynamicRegistryManager()});
   ...
   void openExperimentsScreen(DataConfiguration dataConfiguration) {
      ...
      if (pair != null) {
         this.client.setScreen(new ExperimentsScreen(this, (ResourcePackManager)pair.getSecond(), (resourcePackManager) -> this.applyDataPacks(resourcePackManager, false, this::openExperimentsScreen)));
      }
   ...
   void openPackScreen(DataConfiguration dataConfiguration) {
      Pair<Path, ResourcePackManager> pair = this.getScannedPack(dataConfiguration);
      if (pair != null) {
         this.client.setScreen(new PackScreen((ResourcePackManager)pair.getSecond(), (resourcePackManager) -> this.applyDataPacks(resourcePackManager, true, this::openPackScreen), (Path)pair.getFirst(), Text.translatable("dataPack.title")));
      }
   ...
   private void validateDataPacks(ResourcePackManager dataPackManager, DataConfiguration dataConfiguration, Consumer configurationSetter) {
      ...
      CompletableFuture var10000 = SaveLoading.load(serverConfig, (context) -> {
         if (context.worldGenRegistryManager().get(RegistryKeys.WORLD_PRESET).size() == 0) {
            ...
            WorldGenSettings worldGenSettings = (WorldGenSettings)dataResult.flatMap((json) -> WorldGenSettings.CODEC.parse(dynamicOps2, json)).getOrThrow((string) -> new IllegalStateException("Error parsing worldgen settings after loading data packs: " + string));
   ...
   private static SaveLoading.ServerConfig createServerConfig(ResourcePackManager dataPackManager, DataConfiguration dataConfiguration) {
      ...
      return new SaveLoading.ServerConfig(dataPacks, RegistrationEnvironment.INTEGRATED, 2);
   }
   ...
   private Optional createSession() {
      ...
      } catch (UncheckedIOException | IOException exception2) {
   ...
   class GameTab extends GridScreenTab {
      ...
      GameTab() {
         ...
         CreateWorldScreen.this.worldCreator.addListener((creator) -> this.worldNameField.setTooltip(Tooltip.of(Text.translatable("selectWorld.targetFolder", new Object[]{Text.literal(creator.getWorldDirectoryName()).formatted(Formatting.ITALIC)}))));
         ...
         CyclingButtonWidget<WorldCreator.Mode> cyclingButtonWidget = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder((value) -> value.name).values((Object[])(WorldCreator.Mode.SURVIVAL, WorldCreator.Mode.HARDCORE, WorldCreator.Mode.CREATIVE)).build(0, 0, 210, 20, CreateWorldScreen.GAME_MODE_TEXT, (button, value) -> CreateWorldScreen.this.worldCreator.setGameMode(value)), positioner);
         ...
         CyclingButtonWidget<Difficulty> cyclingButtonWidget2 = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder(Difficulty::getTranslatableName).values((Object[])Difficulty.values()).build(0, 0, 210, 20, Text.translatable("options.difficulty"), (button, value) -> CreateWorldScreen.this.worldCreator.setDifficulty(value)), positioner);
         CreateWorldScreen.this.worldCreator.addListener((creator) -> {
         ...
         CyclingButtonWidget<Boolean> cyclingButtonWidget3 = (CyclingButtonWidget)adder.add(CyclingButtonWidget.onOffBuilder().tooltip((value) -> Tooltip.of(CreateWorldScreen.ALLOW_COMMANDS_INFO_TEXT)).build(0, 0, 210, 20, ALLOW_COMMANDS_TEXT, (button, value) -> CreateWorldScreen.this.worldCreator.setCheatsEnabled(value)));
         CreateWorldScreen.this.worldCreator.addListener((creator) -> {
         ...
         if (!SharedConstants.getGameVersion().isStable()) {
            adder.add(ButtonWidget.builder(CreateWorldScreen.EXPERIMENTS_TEXT, (button) -> CreateWorldScreen.this.openExperimentsScreen(CreateWorldScreen.this.worldCreator.getGeneratorOptionsHolder().dataConfiguration())).width(210).build());
         }
   ...
   class WorldTab extends GridScreenTab {
      ...
      WorldTab() {
         ...
         CyclingButtonWidget<WorldCreator.WorldType> cyclingButtonWidget = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder(WorldCreator.WorldType::getName).values(this.getWorldTypes()).narration(WorldTab::getWorldTypeNarrationMessage).build(0, 0, 150, 20, Text.translatable("selectWorld.mapType"), (button, worldType) -> CreateWorldScreen.this.worldCreator.setWorldType(worldType)));
         ...
         this.customizeButton = (ButtonWidget)adder.add(ButtonWidget.builder(Text.translatable("selectWorld.customizeType"), (button) -> this.openCustomizeScreen()).build());
         ...
         builder.add(var10001, var7, var10003::setGenerateStructures).toggleable(() -> !CreateWorldScreen.this.worldCreator.isDebug()).tooltip(MAP_FEATURES_INFO_TEXT);
         var10001 = BONUS_ITEMS_TEXT;
         WorldCreator var8 = CreateWorldScreen.this.worldCreator;
         Objects.requireNonNull(var8);
         BooleanSupplier var9 = var8::isBonusChestEnabled;
         var10003 = CreateWorldScreen.this.worldCreator;
         Objects.requireNonNull(var10003);
         builder.add(var10001, var9, var10003::setBonusChestEnabled).toggleable(() -> !CreateWorldScreen.this.worldCreator.isHardcore() && !CreateWorldScreen.this.worldCreator.isDebug());
      ...
      private static MutableText getWorldTypeNarrationMessage(CyclingButtonWidget worldTypeButton) {
         return ((WorldCreator.WorldType)worldTypeButton.getValue()).isAmplified() ? ScreenTexts.joinSentences(new Text[]{worldTypeButton.getGenericNarrationMessage(), AMPLIFIED_GENERATOR_INFO_TEXT}) : worldTypeButton.getGenericNarrationMessage();
      }
