...
public class RecipeAlternativesWidget implements Drawable, Element {
   ...
   public void showAlternativesForResult(MinecraftClient client, RecipeResultCollection results, int buttonX, int buttonY, int areaCenterX, int areaCenterY, float delta) {
      ...
      for(int p = 0; p < j; ++p) {
         ...
         if (this.furnace) {
            this.alternativeButtons.add(new FurnaceAlternativeButtonWidget(q, r, recipeEntry, bl2));
         } else {
   ...
   @Environment(EnvType.CLIENT)
   class AlternativeButtonWidget extends ClickableWidget implements RecipeGridAligner {
      ...
      public void acceptAlignedInput(Ingredient ingredient, int i, int j, int k, int l) {
         ...
         if (itemStacks.length != 0) {
            this.slots.add(new InputSlot(3 + k * 7, 3 + l * 7, itemStacks));
         }
      ...
      public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
         ...
         if (this.craftable) {
            ...
            identifier = this.isSelected() ? RecipeAlternativesWidget.FURNACE_OVERLAY_DISABLED_HIGHLIGHTED_TEXTURE : RecipeAlternativesWidget.FURNACE_OVERLAY_DISABLED_TEXTURE;
         } else {
            identifier = this.isSelected() ? RecipeAlternativesWidget.CRAFTING_OVERLAY_DISABLED_HIGHLIGHTED_TEXTURE : RecipeAlternativesWidget.CRAFTING_OVERLAY_DISABLED_TEXTURE;
         }

         context.drawGuiTexture(identifier, this.getX(), this.getY(), this.width, this.height);
         context.getMatrices().push();
         context.getMatrices().translate((double)(this.getX() + 2), (double)(this.getY() + 2), (double)150.0F);

         for(InputSlot inputSlot : this.slots) {
            context.getMatrices().push();
            context.getMatrices().translate((double)inputSlot.y, (double)inputSlot.x, (double)0.0F);
            context.getMatrices().scale(0.375F, 0.375F, 1.0F);
            context.getMatrices().translate((double)-8.0F, (double)-8.0F, (double)0.0F);
   ...
   @Environment(EnvType.CLIENT)
   class FurnaceAlternativeButtonWidget extends AlternativeButtonWidget {
      ...
      protected void alignRecipe(RecipeEntry recipe) {
         Ingredient ingredient = (Ingredient)recipe.value().getIngredients().get(0);
         ItemStack[] itemStacks = ingredient.getMatchingStacks();
