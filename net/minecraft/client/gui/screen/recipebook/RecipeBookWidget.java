...
public class RecipeBookWidget implements RecipeGridAligner, Drawable, Element, Selectable, RecipeDisplayListener {
   ...
   public void reset() {
      ...
      if (this.currentTab != null) {
         this.currentTab = (RecipeGroupButtonWidget)this.tabButtons.stream().filter((button) -> button.getCategory().equals(this.currentTab.getCategory())).findFirst().orElse((Object)null);
      }
   ...
   private void refreshResults(boolean resetCurrentPage) {
      ...
      if (!string.isEmpty()) {
         ...
         if (clientPlayNetworkHandler != null) {
            ObjectSet<RecipeResultCollection> objectSet = new ObjectLinkedOpenHashSet(clientPlayNetworkHandler.getSearchManager().getRecipeOutputReloadFuture().findAll(string.toLowerCase(Locale.ROOT)));
   ...
   public void showGhostRecipe(RecipeEntry recipe, List slots) {
      ItemStack itemStack = recipe.value().getResult(this.client.world.getRegistryManager());
      this.ghostSlots.setRecipe(recipe);
      this.ghostSlots.addSlot(Ingredient.ofStacks(new ItemStack[]{itemStack}), ((Slot)slots.get(0)).x, ((Slot)slots.get(0)).y);
   ...
   public void acceptAlignedInput(Ingredient ingredient, int i, int j, int k, int l) {
      if (!ingredient.isEmpty()) {
         Slot slot = (Slot)this.craftingScreenHandler.slots.get(i);
