...
public class RecipeBookResults {
   ...
   public void setResults(List resultCollections, boolean resetCurrentPage) {
      ...
      this.pageCount = (int)Math.ceil((double)resultCollections.size() / (double)20.0F);
   ...
   public void draw(DrawContext context, int x, int y, int mouseX, int mouseY, float delta) {
      if (this.pageCount > 1) {
         ...
         context.drawText(this.client.textRenderer, (Text)text, x - i / 2 + 73, y + 141, -1, false);
      }
         ...
         for(AnimatedResultButton animatedResultButton : this.resultButtons) {
            if (animatedResultButton.mouseClicked(mouseX, mouseY, button)) {
               if (button == 0) {
                  ...
                  this.alternatesWidget.showAlternativesForResult(this.client, animatedResultButton.getResultCollection(), animatedResultButton.getX(), animatedResultButton.getY(), areaLeft + areaWidth / 2, areaTop + 13 + areaHeight / 2, (float)animatedResultButton.getWidth());
               }
