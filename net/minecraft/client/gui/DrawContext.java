...
public class DrawContext {
   ...
   public int drawText(TextRenderer textRenderer, @Nullable String text, int x, int y, int color, boolean shadow) {
      if (text == null) {
         ...
         int i = textRenderer.draw(text, (float)x, (float)y, color, shadow, this.matrices.peek().getPositionMatrix(), this.vertexConsumers, TextRenderer.TextLayerType.NORMAL, 0, 15728880, textRenderer.isRightToLeft());
         this.tryDraw();
         return i;
      }
   }
   ...
   public int drawText(TextRenderer textRenderer, OrderedText text, int x, int y, int color, boolean shadow) {
      int i = textRenderer.draw((OrderedText)text, (float)x, (float)y, color, shadow, this.matrices.peek().getPositionMatrix(), this.vertexConsumers, TextRenderer.TextLayerType.NORMAL, 0, 15728880);
      this.tryDraw();
      return i;
   }
   ...
   public int drawTextWithBackground(TextRenderer textRenderer, Text text, int x, int y, int width, int color) {
      ...
      if (i != 0) {
         ...
         Objects.requireNonNull(textRenderer);
         this.fill(var10001, var10002, var10003, y + 9 + 2, Argb.mixColor(i, color));
      }
   ...
   private void drawSprite(Sprite sprite, Scaling.NineSlice nineSlice, int x, int y, int z, int width, int height) {
      ...
      if (width == nineSlice.width() && height == nineSlice.height()) {
         ...
         this.drawSpriteTiled(sprite, x + i, y + height - l, z, width - j - i, l, i, nineSlice.height() - l, nineSlice.width() - j - i, l, nineSlice.width(), nineSlice.height());
         ...
         this.drawSpriteTiled(sprite, x + i, y + k, z, width - j - i, height - l - k, i, k, nineSlice.width() - j - i, nineSlice.height() - l - k, nineSlice.width(), nineSlice.height());
         this.drawSpriteTiled(sprite, x + width - j, y + k, z, i, height - l - k, nineSlice.width() - j, k, j, nineSlice.height() - l - k, nineSlice.width(), nineSlice.height());
      }
   }
   ...
   private void drawTooltip(TextRenderer textRenderer, List components, int x, int y, TooltipPositioner positioner) {
      if (!components.isEmpty()) {
         ...
         this.matrices.push();
         int p = 400;
         this.draw(() -> TooltipBackgroundRenderer.render(this, n, o, i, j, 400));
   ...
   public void drawHoverEvent(TextRenderer textRenderer, @Nullable Style style, int x, int y) {
      if (style != null && style.getHoverEvent() != null) {
         ...
         HoverEvent.ItemStackContent itemStackContent = (HoverEvent.ItemStackContent)hoverEvent.getValue(Action.SHOW_ITEM);
         if (itemStackContent != null) {
            this.drawItemTooltip(textRenderer, itemStackContent.asStack(), x, y);
         } else {
            HoverEvent.EntityContent entityContent = (HoverEvent.EntityContent)hoverEvent.getValue(Action.SHOW_ENTITY);
