...
public abstract class ClickableWidget implements Drawable, Element, Widget, Selectable {
   private static final double field_43055 = (double)0.5F;
   private static final double field_43056 = (double)3.0F;
   protected int width;
   ...
   protected static void drawScrollableText(DrawContext context, TextRenderer textRenderer, Text text, int centerX, int startX, int startY, int endX, int endY, int color) {
      ...
      if (i > k) {
         ...
         double e = Math.max((double)l * (double)0.5F, (double)3.0F);
         double f = Math.sin((Math.PI / 2D) * Math.cos((Math.PI * 2D) * d / e)) / (double)2.0F + (double)0.5F;
         double g = MathHelper.lerp(f, (double)0.0F, (double)l);
