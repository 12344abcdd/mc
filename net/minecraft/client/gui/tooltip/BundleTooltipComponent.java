...
public class BundleTooltipComponent implements TooltipComponent {
   ...
   private int getColumns() {
      return Math.max(2, (int)Math.ceil(Math.sqrt((double)this.bundleContents.size() + (double)1.0F)));
   }

   private int getRows() {
      return (int)Math.ceil(((double)this.bundleContents.size() + (double)1.0F) / (double)this.getColumns());
   }
   ...
   static enum SlotSprite {
      ...
      // $FF: synthetic method
      private static SlotSprite[] method_36887() {
         return new SlotSprite[]{BLOCKED_SLOT, SLOT};
      }
   }
}
