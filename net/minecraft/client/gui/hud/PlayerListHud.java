...
public class PlayerListHud {
   ...
   private static final Comparator ENTRY_ORDERING = Comparator.comparingInt((entry) -> entry.getGameMode() == GameMode.SPECTATOR ? 1 : 0).thenComparing((entry) -> (String)Nullables.mapOrElse(entry.getScoreboardTeam(), Team::getName, "")).thenComparing((entry) -> entry.getProfile().getName(), String::compareToIgnoreCase);
   ...
   public void setVisible(boolean visible) {
      if (this.visible != visible) {
         ...
         if (visible) {
            ...
            this.client.getNarratorManager().narrate((Text)Text.translatable("multiplayer.player.list.narration", new Object[]{text}));
         }
      }
   ...
   public void render(DrawContext context, int scaledWindowWidth, Scoreboard scoreboard, @Nullable ScoreboardObjective objective) {
      ...
      List<ScoreDisplayEntry> list2 = new ArrayList(list.size());
      ...
      if (list4 != null) {
         s += o * 9 + 1;
         int var56 = scaledWindowWidth / 2 - t / 2 - 1;
         int var57 = s - 1;
         int var58 = scaledWindowWidth / 2 + t / 2 + 1;
         int var59 = list4.size();
         Objects.requireNonNull(this.client.textRenderer);
         context.fill(var56, var57, var58, s + var59 * 9, Integer.MIN_VALUE);
   ...
   private void renderHearts(int y, int left, int right, UUID uuid, DrawContext context, int score) {
      Heart heart = (Heart)this.hearts.computeIfAbsent(uuid, (uuid2) -> new Heart(score));
      ...
      if (i > 0) {
         ...
         if (k <= 3) {
            ...
            context.drawTextWithShadow(this.client.textRenderer, text2, (right + left - this.client.textRenderer.getWidth((StringVisitable)text2)) / 2, y, l);
         } else {
