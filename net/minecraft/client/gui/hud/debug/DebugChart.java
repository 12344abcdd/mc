...
public abstract class DebugChart {
   ...
   protected void drawBorderedText(DrawContext context, String string, int x, int y) {
      ...
      int var10004 = x + this.textRenderer.getWidth(string) + 1;
      Objects.requireNonNull(this.textRenderer);
      context.fill(var10001, x, y, var10004, y + 9, -1873784752);
   ...
   protected int getColor(double value, double min, int minColor, double median, int medianColor, double max, int maxColor) {
      ...
      return value < median ? Argb.lerp((float)((value - min) / (median - min)), minColor, medianColor) : Argb.lerp((float)((value - median) / (max - median)), medianColor, maxColor);
   }
}
