...
public class TickChart extends DebugChart {
   ...
   protected int getHeight(double value) {
      return (int)Math.round(toMillisecondsPerTick(value) * (double)60.0F / (double)(Float)this.millisPerTickSupplier.get());
   }

   protected int getColor(long value) {
      float f = (Float)this.millisPerTickSupplier.get();
      return this.getColor(toMillisecondsPerTick((double)value), (double)f, -16711936, (double)f * (double)1.125F, -256, (double)f * (double)1.25F, -65536);
   }

   private static double toMillisecondsPerTick(double nanosecondsPerTick) {
      return nanosecondsPerTick / (double)1000000.0F;
   }
}
