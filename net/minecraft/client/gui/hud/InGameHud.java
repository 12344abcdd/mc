...
public class InGameHud {
   ...
   public InGameHud(MinecraftClient client) {
      ...
      LayeredDrawer layeredDrawer = (new LayeredDrawer()).addLayer(this::renderMiscOverlays).addLayer(this::renderCrosshair).addLayer(this::renderMainHud).addLayer(this::renderExperienceLevel).addLayer(this::renderStatusEffectOverlay).addLayer((context, tickCounter) -> this.bossBarHud.render(context));
      LayeredDrawer layeredDrawer2 = (new LayeredDrawer()).addLayer(this::renderDemoTimer).addLayer((context, tickCounter) -> {
      ...
      this.layeredDrawer.addSubDrawer(layeredDrawer, () -> !client.options.hudHidden).addLayer(this::renderSleepOverlay).addSubDrawer(layeredDrawer2, () -> !client.options.hudHidden);
   }
   ...
   private void renderStatusEffectOverlay(DrawContext context, RenderTickCounter tickCounter) {
      ...
      if (!collection.isEmpty()) {
         Screen var5 = this.client.currentScreen;
         if (var5 instanceof AbstractInventoryScreen) {
            AbstractInventoryScreen abstractInventoryScreen = (AbstractInventoryScreen)var5;
         ...
         for(StatusEffectInstance statusEffectInstance : Ordering.natural().reverse().sortedCopy(collection)) {
            ...
            if (statusEffectInstance.shouldShowIcon()) {
               ...
               list.add((Runnable)() -> {
                  ...
                  context.drawSprite(k + 3, l + 3, 0, 18, 18, sprite);
   ...
   private void renderExperienceLevel(DrawContext context, RenderTickCounter tickCounter) {
      ...
      if (this.shouldRenderExperience() && i > 0) {
         ...
         context.drawText(this.getTextRenderer(), (String)string, j + 1, k, 0, false);
         context.drawText(this.getTextRenderer(), (String)string, j - 1, k, 0, false);
         context.drawText(this.getTextRenderer(), (String)string, j, k + 1, 0, false);
         context.drawText(this.getTextRenderer(), (String)string, j, k - 1, 0, false);
   ...
   private void renderHeldItemTooltip(DrawContext context) {
      ...
      if (this.heldItemTooltipFade > 0 && !this.currentStack.isEmpty()) {
         ...
         if (l > 0) {
            context.drawTextWithBackground(this.getTextRenderer(), mutableText, j, k, i, Argb.withAlpha(l, -1));
         }
      }
   ...
   private void renderDemoTimer(DrawContext context, RenderTickCounter tickCounter) {
      if (this.client.isDemo()) {
         ...
         if (this.client.world.getTime() >= 120500L) {
            ...
            text = Text.translatable("demo.remainingTime", new Object[]{StringHelper.formatTicks((int)(120500L - this.client.world.getTime()), this.client.world.getTickManager().getTickRate())});
         }
   ...
   private void renderScoreboardSidebar(DrawContext context, ScoreboardObjective objective) {
      ...
      context.draw(() -> {
         int k = sidebarEntrys.length;
         Objects.requireNonNull(this.getTextRenderer());
         int l = k * 9;
         int m = context.getScaledWindowHeight() / 2 + l / 3;
         int n = 3;
         int o = context.getScaledWindowWidth() - j - 3;
         ...
         int var10001 = o - 2;
         Objects.requireNonNull(this.getTextRenderer());
         context.fill(var10001, s - 9 - 1, p, s - 1, r);
         context.fill(o - 2, s - 1, p, m, q);
         TextRenderer var18 = this.getTextRenderer();
         int var10003 = o + j / 2 - i / 2;
         Objects.requireNonNull(this.getTextRenderer());
         context.drawText(var18, (Text)text, var10003, s - 9, -1, false);

         for(int t = 0; t < k; ++t) {
            SidebarEntry sidebarEntry = sidebarEntrys[t];
            int var19 = k - t;
            Objects.requireNonNull(this.getTextRenderer());
            int u = m - var19 * 9;
            context.drawText(this.getTextRenderer(), (Text)sidebarEntry.name, o, u, -1, false);
            context.drawText(this.getTextRenderer(), (Text)sidebarEntry.score, p - sidebarEntry.scoreWidth, u, -1, false);
         }
   ...
   private void renderStatusBars(DrawContext context) {
      ...
      if (playerEntity != null) {
         ...
         if (playerEntity.isSubmergedIn(FluidTags.WATER) || v < u) {
            ...
            r -= w * 10;
            int x = MathHelper.ceil((double)(v - 2) * (double)10.0F / (double)u);
            int y = MathHelper.ceil((double)v * (double)10.0F / (double)u) - x;
            RenderSystem.enableBlend();
   ...
   private void renderVignetteOverlay(DrawContext context, @Nullable Entity entity) {
      ...
      if (entity != null) {
         ...
         double d = Math.min(worldBorder.getShrinkingSpeed() * (double)worldBorder.getWarningTime() * (double)1000.0F, Math.abs(worldBorder.getSizeLerpTarget() - worldBorder.getSize()));
   ...
   private void tick() {
      ...
      if (this.client.player != null) {
         ...
         if (itemStack.isEmpty()) {
            ...
            this.heldItemTooltipFade = (int)((double)40.0F * (Double)this.client.options.getNotificationDisplayTime().getValue());
         }
   ...
   public void renderAutosaveIndicator(DrawContext context, RenderTickCounter tickCounter) {
      if ((Boolean)this.client.options.getShowAutosaveIndicator().getValue() && (this.autosaveIndicatorAlpha > 0.0F || this.lastAutosaveIndicatorAlpha > 0.0F)) {
         int i = MathHelper.floor(255.0F * MathHelper.clamp(MathHelper.lerp(tickCounter.getLastDuration(), this.lastAutosaveIndicatorAlpha, this.autosaveIndicatorAlpha), 0.0F, 1.0F));
         if (i > 8) {
            TextRenderer textRenderer = this.getTextRenderer();
