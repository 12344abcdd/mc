...
public class TextureStitcher {
   private static final Comparator COMPARATOR = Comparator.comparing((holder) -> -holder.height).thenComparing((holder) -> -holder.width).thenComparing((holder) -> holder.sprite.getId());
   ...
   public void add(Stitchable info) {
      Holder<T> holder = new Holder(info, this.mipLevel);
      this.holders.add(holder);
   }

   public void stitch() {
      List<Holder<T>> list = new ArrayList(this.holders);
      list.sort(COMPARATOR);

      for(Holder holder : list) {
         if (!this.fit(holder)) {
            throw new TextureStitcherCannotFitException(holder.sprite, (Collection)list.stream().map((holderx) -> holderx.sprite).collect(ImmutableList.toImmutableList()));
         }
      }
   ...
   @Environment(EnvType.CLIENT)
   static record Holder(Stitchable sprite, int width, int height) {
