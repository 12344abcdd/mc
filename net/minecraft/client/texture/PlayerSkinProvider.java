...
public class PlayerSkinProvider {
   ...
   public PlayerSkinProvider(TextureManager textureManager, Path directory, final MinecraftSessionService sessionService, final Executor executor) {
      ...
      this.skinCache = new FileCache(textureManager, directory, Type.SKIN);
      this.capeCache = new FileCache(textureManager, directory, Type.CAPE);
      this.elytraCache = new FileCache(textureManager, directory, Type.ELYTRA);
   ...
   public SkinTextures getSkinTextures(GameProfile profile) {
      SkinTextures skinTextures = (SkinTextures)this.fetchSkinTextures(profile).getNow((Object)null);
   ...
   CompletableFuture fetchSkinTextures(UUID uuid, MinecraftProfileTextures textures) {
      ...
      CompletableFuture<Identifier> completableFuture2 = minecraftProfileTexture2 != null ? this.capeCache.get(minecraftProfileTexture2) : CompletableFuture.completedFuture((Object)null);
      MinecraftProfileTexture minecraftProfileTexture3 = textures.elytra();
      CompletableFuture<Identifier> completableFuture3 = minecraftProfileTexture3 != null ? this.elytraCache.get(minecraftProfileTexture3) : CompletableFuture.completedFuture((Object)null);
   ...
   static class FileCache {
      ...
      public CompletableFuture get(MinecraftProfileTexture texture) {
         ...
         CompletableFuture<Identifier> completableFuture = (CompletableFuture)this.hashToTexture.get(string);
      ...
      private CompletableFuture store(MinecraftProfileTexture texture) {
         ...
         PlayerSkinTexture playerSkinTexture = new PlayerSkinTexture(path.toFile(), texture.getUrl(), DefaultSkinHelper.getTexture(), this.type == Type.SKIN, () -> completableFuture.complete(identifier));
