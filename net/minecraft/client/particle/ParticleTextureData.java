...
public class ParticleTextureData {
   ...
   public static ParticleTextureData load(JsonObject json) {
      ...
      if (jsonArray == null) {
         ...
         List<Identifier> list = (List)Streams.stream(jsonArray).map((texture) -> JsonHelper.asString(texture, "texture")).map(Identifier::of).collect(ImmutableList.toImmutableList());
