...
public class NoteParticle extends SpriteBillboardParticle {
   NoteParticle(ClientWorld world, double x, double y, double z, double d) {
      ...
      this.velocityY += 0.2;
      this.red = Math.max(0.0F, MathHelper.sin(((float)d + 0.0F) * ((float)Math.PI * 2F)) * 0.65F + 0.35F);
      this.green = Math.max(0.0F, MathHelper.sin(((float)d + 0.33333334F) * ((float)Math.PI * 2F)) * 0.65F + 0.35F);
      this.blue = Math.max(0.0F, MathHelper.sin(((float)d + 0.6666667F) * ((float)Math.PI * 2F)) * 0.65F + 0.35F);
      this.scale *= 1.5F;
      this.maxAge = 6;
   }
