...
public class PendingUpdateManager implements AutoCloseable {
   ...
   public void addPendingUpdate(BlockPos pos, BlockState state, ClientPlayerEntity player) {
      this.blockPosToPendingUpdate.compute(pos.asLong(), (posLong, pendingUpdate) -> pendingUpdate != null ? pendingUpdate.withSequence(this.sequence) : new PendingUpdate(this.sequence, state, player.getPos()));
   }
   ...
   public void processPendingUpdates(int maxProcessableSequence, ClientWorld world) {
      ObjectIterator<Long2ObjectMap.Entry<PendingUpdate>> objectIterator = this.blockPosToPendingUpdate.long2ObjectEntrySet().iterator();

      while(objectIterator.hasNext()) {
         Long2ObjectMap.Entry<PendingUpdate> entry = (Long2ObjectMap.Entry)objectIterator.next();
