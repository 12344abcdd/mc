...
public abstract class ClientCommonNetworkHandler implements ClientCommonPacketListener {
   ...
   public void onPacketException(Packet packet, Exception exception) {
      ...
      Optional<URI> optional2 = this.serverLinks.getEntryFor(Known.BUG_REPORT).map(ServerLinks.Entry::link);
   ...
   public DisconnectionInfo createDisconnectionInfo(Text reason, Throwable exception) {
      Optional<Path> optional = this.savePacketErrorReport((Packet)null, exception);
      Optional<URI> optional2 = this.serverLinks.getEntryFor(Known.BUG_REPORT).map(ServerLinks.Entry::link);
   ...
   private Optional savePacketErrorReport(@Nullable Packet packet, Throwable exception) {
      ...
      Optional<ServerLinks.Entry> optional = this.serverLinks.getEntryFor(Known.BUG_REPORT);
      List<String> list = (List)optional.map((bugReportEntry) -> List.of("Server bug reporting link: " + String.valueOf(bugReportEntry.link()))).orElse(List.of());
   ...
   static Text getPrompt(Text requirementPrompt, @Nullable Text customPrompt) {
      return (Text)(customPrompt == null ? requirementPrompt : Text.translatable("multiplayer.texturePrompt.serverPrompt", new Object[]{requirementPrompt, customPrompt}));
   }
   ...
   public void onServerLinks(ServerLinksS2CPacket packet) {
      ...
      ImmutableList.Builder<ServerLinks.Entry> builder = ImmutableList.builderWithExpectedSize(list.size());
   ...
   protected Screen createDisconnectedScreen(DisconnectionInfo info) {
      ...
      return (Screen)(this.serverInfo != null && this.serverInfo.isRealm() ? new DisconnectedRealmsScreen(screen, LOST_CONNECTION_TEXT, info.reason()) : new DisconnectedScreen(screen, LOST_CONNECTION_TEXT, info));
   }
   ...
   class ConfirmServerResourcePackScreen extends ConfirmScreen {
      ...
      ConfirmServerResourcePackScreen(final MinecraftClient client, @Nullable final Screen parent, final List pack, final boolean required, @Nullable final Text prompt) {
         ...
         }, required ? Text.translatable("multiplayer.requiredTexturePrompt.line1") : Text.translatable("multiplayer.texturePrompt.line1"), ClientCommonNetworkHandler.getPrompt(required ? Text.translatable("multiplayer.requiredTexturePrompt.line2").formatted(new Formatting[]{Formatting.YELLOW, Formatting.BOLD}) : Text.translatable("multiplayer.texturePrompt.line2"), prompt), required ? ScreenTexts.PROCEED : ScreenTexts.YES, required ? ScreenTexts.DISCONNECT : ScreenTexts.NO);
         this.packs = pack;
         this.parent = parent;
      }
      ...
      public ConfirmServerResourcePackScreen add(MinecraftClient client, UUID id, URL url, String hash, boolean required, @Nullable Text prompt) {
         List<Pack> list = ImmutableList.builderWithExpectedSize(this.packs.size() + 1).addAll(this.packs).add(new Pack(id, url, hash)).build();
