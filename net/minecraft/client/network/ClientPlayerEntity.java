...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
   ...
   private void sendMovementPackets() {
      ...
      if (this.isCamera()) {
         ...
         if (this.hasVehicle()) {
            ...
            this.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(vec3d.x, (double)-999.0F, vec3d.z, this.getYaw(), this.getPitch(), this.isOnGround()));
            bl2 = false;
         } else if (bl2 && bl3) {
   ...
   public boolean dropSelectedItem(boolean entireStack) {
      PlayerActionC2SPacket.Action action = entireStack ? Action.DROP_ALL_ITEMS : Action.DROP_ITEM;
   ...
   private void pushOutOfBlocks(double x, double z) {
      ...
      if (this.wouldCollideAt(blockPos)) {
         ...
         for(Direction direction2 : directions) {
            double g = direction2.getAxis().choose(d, (double)0.0F, e);
            double h = direction2.getDirection() == AxisDirection.POSITIVE ? (double)1.0F - g : g;
   ...
   private boolean wouldCollideAt(BlockPos pos) {
      ...
      Box box2 = (new Box((double)pos.getX(), box.minY, (double)pos.getZ(), (double)pos.getX() + (double)1.0F, box.maxY, (double)pos.getZ() + (double)1.0F)).contract(1.0E-7);
   ...
   public void tickMovement() {
      ...
      if (playerAbilities.flying && this.isCamera()) {
         ...
         if (i != 0) {
            this.setVelocity(this.getVelocity().add((double)0.0F, (double)((float)i * playerAbilities.getFlySpeed() * 3.0F), (double)0.0F));
         }
      }
   ...
   @Nullable
   public StatusEffectInstance removeStatusEffectInternal(RegistryEntry effect) {
   ...
   protected void autoJump(float dx, float dz) {
      if (this.shouldAutoJump()) {
         ...
         Vec3d vec3d2 = vec3d.add((double)dx, (double)0.0F, (double)dz);
         ...
         if (g <= 0.001F) {
            ...
            float h = f * vec2f.x;
            float i = f * vec2f.y;
            float j = MathHelper.sin(this.getYaw() * ((float)Math.PI / 180F));
            float k = MathHelper.cos(this.getYaw() * ((float)Math.PI / 180F));
         ...
         if (!(j < -0.15F)) {
            ...
            if (blockState.getCollisionShape(this.getWorld(), blockPos, shapeContext).isEmpty()) {
               ...
               if (blockState2.getCollisionShape(this.getWorld(), blockPos, shapeContext).isEmpty()) {
                  ...
                  Box box = (new Box(vec3d, vec3d7.add((double)0.0F, (double)q, (double)0.0F))).expand((double)p, (double)0.0F, (double)p);
                  Vec3d vec3d6 = vec3d.add((double)0.0F, (double)0.51F, (double)0.0F);
                  vec3d7 = vec3d7.add((double)0.0F, (double)0.51F, (double)0.0F);
                  Vec3d vec3d8 = vec3d4.crossProduct(new Vec3d((double)0.0F, (double)1.0F, (double)0.0F));
                     ...
                     if (box2.intersects(vec3d10, vec3d11) || box2.intersects(vec3d12, vec3d13)) {
                        ...
                        for(int s = 1; (float)s < n; ++s) {
                           ...
                           if (!(voxelShape = blockState3.getCollisionShape(this.getWorld(), blockPos3, shapeContext)).isEmpty()) {
                              r = (float)voxelShape.getMax(Axis.Y) + (float)blockPos3.getY();
   ...
   private boolean shouldAutoJump() {
      return this.isAutoJumpEnabled() && this.ticksToNextAutojump <= 0 && this.isOnGround() && !this.clipAtLedge() && !this.hasVehicle() && this.hasMovementInput() && (double)this.getJumpVelocityMultiplier() >= (double)1.0F;
   }
   ...
   public void onGameModeChanged(GameMode gameMode) {
      if (gameMode == GameMode.SPECTATOR) {
         this.setVelocity(this.getVelocity().withAxis(Axis.Y, (double)0.0F));
      }
   ...
   public Vec3d getLeashPos(float delta) {
      if (this.client.options.getPerspective().isFirstPerson()) {
         float f = MathHelper.lerp(delta * 0.5F, this.getYaw(), this.prevYaw) * ((float)Math.PI / 180F);
         float g = MathHelper.lerp(delta * 0.5F, this.getPitch(), this.prevPitch) * ((float)Math.PI / 180F);
         double d = this.getMainArm() == Arm.RIGHT ? (double)-1.0F : (double)1.0F;
