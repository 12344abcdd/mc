...
public class Bans {
   ...
   public static ConfirmLinkScreen createUsernameBanScreen(String username, Runnable onClose) {
      ...
      return new ConfirmLinkScreen((confirmed) -> {
         ...
         onClose.run();
      }, NAME_TITLE, Text.translatable("gui.banned.name.description", new Object[]{Text.literal(username).formatted(Formatting.YELLOW), Text.of(Urls.JAVA_MODERATION)}), uRI, ScreenTexts.ACKNOWLEDGE, true);
   }
   ...
   private static Text getDurationText(BanDetails banDetails) {
      if (isTemporary(banDetails)) {
         ...
         return Text.translatable("gui.banned.description.temporary", new Object[]{Text.translatable("gui.banned.description.temporary.duration", new Object[]{text}).formatted(Formatting.BOLD)});
      } else {
   ...
   static {
      ...
      SKIN_DESCRIPTION = Text.translatable("gui.banned.skin.description", new Object[]{Text.of(Urls.JAVA_MODERATION)});
   }
}
