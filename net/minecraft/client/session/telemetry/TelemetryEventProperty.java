...
public record TelemetryEventProperty(String id, String exportKey, Codec codec, PropertyExporter exporter) {
   ...
   public static TelemetryEventProperty ofLongList(String id, String exportKey) {
      return of(id, exportKey, Codec.LONG.listOf().xmap(LongArrayList::new, Function.identity()), (container, key, value) -> container.addProperty(key, (String)value.longStream().mapToObj(String::valueOf).collect(Collectors.joining(";"))));
   }

   public void addTo(PropertyMap map, TelemetryPropertyContainer container) {
      T object = (T)map.get(this);
      if (object != null) {
   ...
   static {
      ...
      SERVER_TYPE = of("server_type", "serverType", TelemetryEventProperty.ServerType.CODEC, (container, exportKey, value) -> container.addProperty(exportKey, value.asString()));
      OPT_IN = ofBoolean("opt_in", "isOptional");
      EVENT_TIMESTAMP_UTC = of("event_timestamp_utc", "eventTimestampUtc", Codecs.INSTANT, (container, exportKey, value) -> container.addProperty(exportKey, DATE_TIME_FORMATTER.format(value)));
      GAME_MODE = of("game_mode", "playerGameMode", TelemetryEventProperty.GameMode.CODEC, (container, exportKey, value) -> container.addProperty(exportKey, value.getRawId()));
