...
public class ChatLog {
   ...
   public static Codec createCodec(int maxSize) {
      return Codec.list(ChatLogEntry.CODEC).comapFlatMap((entries) -> {
         int j = entries.size();
         return j > maxSize ? DataResult.error(() -> "Expected: a buffer of size less than or equal to " + maxSize + " but: " + j + " is greater than " + maxSize) : DataResult.success(new ChatLog(maxSize, entries));
      }, ChatLog::toList);
   }
