...
public class RealmsPrepareConnectionTask extends LongRunningTask {
   ...
   public void run() {
      ...
      Screen screen = (Screen)(bl2 ? this.createResourcePackConfirmationScreen(realmsServerAddress, getResourcePackId(this.server), this::createConnectingScreen) : this.createConnectingScreen(realmsServerAddress));
      setScreen(screen);
   }

   private static UUID getResourcePackId(RealmsServer server) {
      return server.minigameName != null ? UUID.nameUUIDFromBytes(("minigame:" + server.minigameName).getBytes(StandardCharsets.UTF_8)) : UUID.nameUUIDFromBytes(("realms:" + server.name + ":" + server.activeSlot).getBytes(StandardCharsets.UTF_8));
   }
