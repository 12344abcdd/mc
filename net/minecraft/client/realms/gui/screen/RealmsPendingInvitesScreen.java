...
public class RealmsPendingInvitesScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.pendingInvites.thenAcceptAsync((pendingInvites) -> {
         List<PendingInvitationSelectionListEntry> list = pendingInvites.stream().map((invite) -> new PendingInvitationSelectionListEntry(invite)).toList();
   ...
   void handle(int index, boolean accepted) {
      if (index < this.pendingInvitationSelectionList.getEntryCount()) {
         String string = ((PendingInvitationSelectionListEntry)this.pendingInvitationSelectionList.children().get(index)).mPendingInvite.invitationId;
   ...
   @Environment(EnvType.CLIENT)
   class PendingInvitationSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry {
      ...
      private void renderPendingInvitationItem(DrawContext context, PendingInvite invite, int x, int y, int mouseX, int mouseY) {
         context.drawText(RealmsPendingInvitesScreen.this.textRenderer, (String)invite.worldName, x + 38, y + 1, -1, false);
      ...
      public Text getNarration() {
         Text text = ScreenTexts.joinLines(new Text[]{Text.literal(this.mPendingInvite.worldName), Text.literal(this.mPendingInvite.worldOwnerName), RealmsUtil.convertToAgePresentation(this.mPendingInvite.date)});
      ...
      class AcceptButton extends RealmsAcceptRejectButton {
         ...
         protected void render(DrawContext context, int x, int y, boolean showTooltip) {
            context.drawGuiTexture(showTooltip ? RealmsPendingInvitesScreen.ACCEPT_HIGHLIGHTED_ICON_TEXTURE : RealmsPendingInvitesScreen.ACCEPT_ICON_TEXTURE, x, y, 18, 18);
            if (showTooltip) {
      ...
      class RejectButton extends RealmsAcceptRejectButton {
         ...
         protected void render(DrawContext context, int x, int y, boolean showTooltip) {
            context.drawGuiTexture(showTooltip ? RealmsPendingInvitesScreen.REJECT_HIGHLIGHTED_ICON_TEXTURE : RealmsPendingInvitesScreen.REJECT_ICON_TEXTURE, x, y, 18, 18);
            if (showTooltip) {
