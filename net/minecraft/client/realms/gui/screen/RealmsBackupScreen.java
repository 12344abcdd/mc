...
public class RealmsBackupScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.selectionList = (BackupObjectSelectionList)this.layout.addBody(new BackupObjectSelectionList());
      ...
      this.downloadButton = (ButtonWidget)directionalLayoutWidget.add(ButtonWidget.builder(DOWNLOAD_TEXT, (button) -> this.downloadClicked()).build());
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      if (this.noBackups && this.selectionList != null) {
         ...
         context.drawText(var10001, (Text)var10002, var10003, var10004 - 9 / 2, -1, false);
      }
   ...
   @Environment(EnvType.CLIENT)
   class BackupObjectSelectionList extends ElementListWidget {
   ...
   @Environment(EnvType.CLIENT)
   class BackupObjectSelectionListEntry extends ElementListWidget.Entry {
      ...
      public BackupObjectSelectionListEntry(final Backup backup) {
         ...
         if (!backup.changeList.isEmpty()) {
            this.changesButton = ButtonWidget.builder(RealmsBackupScreen.CHANGES_TOOLTIP, (button) -> RealmsBackupScreen.this.client.setScreen(new RealmsBackupInfoScreen(RealmsBackupScreen.this, this.mBackup))).width(8 + RealmsBackupScreen.this.textRenderer.getWidth((StringVisitable)RealmsBackupScreen.CHANGES_TOOLTIP)).narrationSupplier((textSupplier) -> ScreenTexts.joinSentences(new Text[]{Text.translatable("mco.backup.narration", new Object[]{this.getLastModifiedDate()}), (Text)textSupplier.get()})).build();
            this.buttons.add(this.changesButton);
         }

         if (!RealmsBackupScreen.this.serverData.expired) {
            this.restoreButton = ButtonWidget.builder(RealmsBackupScreen.RESTORE_TEXT, (button) -> this.restore()).width(8 + RealmsBackupScreen.this.textRenderer.getWidth((StringVisitable)RealmsBackupScreen.CHANGES_TOOLTIP)).narrationSupplier((textSupplier) -> ScreenTexts.joinSentences(new Text[]{Text.translatable("mco.backup.narration", new Object[]{this.getLastModifiedDate()}), (Text)textSupplier.get()})).build();
      ...
      private void restore() {
         ...
         RealmsBackupScreen.this.client.setScreen(RealmsPopups.createContinuableWarningPopup(RealmsBackupScreen.this, text2, (popup) -> RealmsBackupScreen.this.client.setScreen(new RealmsLongRunningMcoTaskScreen(RealmsBackupScreen.this.parent.getNewScreen(), new LongRunningTask[]{new RestoreTask(this.mBackup, RealmsBackupScreen.this.serverData.id, RealmsBackupScreen.this.parent)}))));
      }
      ...
      public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
         ...
         context.drawText(RealmsBackupScreen.this.textRenderer, (Text)Text.translatable("mco.backup.entry", new Object[]{RealmsUtil.convertToAgePresentation(this.mBackup.lastModifiedDate)}), x, j, l, false);
