...
public class RealmsDownloadLatestWorldScreen extends RealmsScreen {
   ...
   public void init() {
      this.cancelButton = (ButtonWidget)this.addDrawableChild(ButtonWidget.builder(ScreenTexts.CANCEL, (button) -> this.close()).dimensions((this.width - 200) / 2, this.height - 42, 200, 20).build());
   ...
   private void drawDots(DrawContext context) {
      ...
      context.drawText(this.textRenderer, (String)DOTS[this.dotIndex % DOTS.length], this.width / 2 + i / 2 + 5, 50, -1, false);
   }

   private void drawProgressBar(DrawContext context) {
      double d = Math.min((double)this.downloadStatus.bytesWritten / (double)this.downloadStatus.totalBytes, (double)1.0F);
      ...
      context.drawCenteredTextWithShadow(this.textRenderer, (Text)Text.translatable("mco.download.percent", new Object[]{this.progress}), this.width / 2, 84, -1);
   }
   ...
   private void drawDownloadSpeed0(DrawContext context, long bytesPerSecond) {
      if (bytesPerSecond > 0L) {
         ...
         context.drawText(this.textRenderer, (Text)Text.translatable("mco.download.speed", new Object[]{SizeUnit.getUserFriendlyString(bytesPerSecond)}), this.width / 2 + i / 2 + 15, 84, -1, false);
      }
