...
public class RealmsSubscriptionInfoScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.subscription.extend"), (button) -> ConfirmLinkScreen.open(this, (String)Urls.getExtendJavaRealmsUrl(this.serverData.remoteSubscriptionId, this.client.getSession().getUuidOrNull()))).dimensions(this.width / 2 - 100, row(6), 200, 20).build());
      if (this.serverData.expired) {
         this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.delete.button"), (button) -> this.client.setScreen(RealmsPopups.createContinuableWarningPopup(this, Text.translatable("mco.configure.world.delete.question.line1"), (popupScreen) -> this.onDeletionConfirmed()))).dimensions(this.width / 2 - 100, row(10), 200, 20).build());
      } else if (RealmsMainScreen.isSnapshotRealmsEligible() && this.serverData.parentWorldName != null) {
         this.addDrawableChild(new ScrollableTextWidget(this.width / 2 - 100, row(8), 200, 46, Text.translatable("mco.snapshot.subscription.info", new Object[]{this.serverData.parentWorldName}), this.textRenderer));
      } else {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawCenteredTextWithShadow(this.textRenderer, (Text)SUBSCRIPTION_TITLE, this.width / 2, 17, -1);
      context.drawText(this.textRenderer, SUBSCRIPTION_START_LABEL_TEXT, i, row(0), -6250336, false);
      context.drawText(this.textRenderer, (Text)this.startDate, i, row(1), -1, false);
      ...
      context.drawText(this.textRenderer, (Text)this.daysLeft, i, row(4), -1, false);
   }
