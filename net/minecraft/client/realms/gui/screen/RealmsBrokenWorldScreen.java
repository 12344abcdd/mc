...
public class RealmsBrokenWorldScreen extends RealmsScreen {
   ...
   private void addButtons() {
      for(Map.Entry entry : this.serverData.slots.entrySet()) {
         ...
         if (bl) {
            buttonWidget = ButtonWidget.builder(Text.translatable("mco.brokenworld.play"), (button) -> this.client.setScreen(new RealmsLongRunningMcoTaskScreen(this.parent, new LongRunningTask[]{new SwitchSlotTask(this.serverData.id, i, this::play)}))).dimensions(this.getFramePositionX(i), row(8), 80, 20).build();
            buttonWidget.active = !((RealmsWorldOptions)this.serverData.slots.get(i)).empty;
         } else {
            buttonWidget = ButtonWidget.builder(Text.translatable("mco.brokenworld.download"), (button) -> this.client.setScreen(RealmsPopups.createInfoPopup(this, Text.translatable("mco.configure.world.restore.download.question.line1"), (popupScreen) -> this.downloadWorld(i)))).dimensions(this.getFramePositionX(i), row(8), 80, 20).build();
         }
   ...
   public void play() {
      (new Thread(() -> {
         ...
         if (this.serverData.state == RealmsServer.State.CLOSED) {
            this.client.execute(() -> this.client.setScreen(new RealmsLongRunningMcoTaskScreen(this, new LongRunningTask[]{new OpenServerTask(this.serverData, this, true, this.client)})));
         } else {
            try {
