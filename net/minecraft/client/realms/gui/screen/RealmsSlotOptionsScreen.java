...
public class RealmsSlotOptionsScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      CyclingButtonWidget<Boolean> cyclingButtonWidget = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.pvp).build(i, row(1), this.column2_x, 20, Text.translatable("mco.configure.world.pvp"), (button, pvp) -> this.pvp = pvp));
      this.addDrawableChild(CyclingButtonWidget.builder(GameMode::getSimpleTranslatableName).values((Collection)GAME_MODES).initially(this.gameMode).build(this.column1_x, row(3), this.column2_x, 20, Text.translatable("selectWorld.gameMode"), (button, gameModeIndex) -> this.gameMode = gameModeIndex));
      Text text2 = Text.translatable("mco.configure.world.spawn_toggle.message");
      CyclingButtonWidget<Boolean> cyclingButtonWidget2 = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.spawnAnimals).build(i, row(3), this.column2_x, 20, Text.translatable("mco.configure.world.spawnAnimals"), this.getSpawnToggleButtonCallback(text2, (spawnAnimals) -> this.spawnAnimals = spawnAnimals)));
      CyclingButtonWidget<Boolean> cyclingButtonWidget3 = CyclingButtonWidget.onOffBuilder(this.difficulty != Difficulty.PEACEFUL && this.spawnMonsters).build(i, row(5), this.column2_x, 20, Text.translatable("mco.configure.world.spawnMonsters"), this.getSpawnToggleButtonCallback(text2, (spawnMonsters) -> this.spawnMonsters = spawnMonsters));
      ...
      CyclingButtonWidget<Boolean> cyclingButtonWidget4 = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.spawnNpcs).build(i, row(7), this.column2_x, 20, Text.translatable("mco.configure.world.spawnNPCs"), this.getSpawnToggleButtonCallback(Text.translatable("mco.configure.world.spawn_toggle.message.npc"), (spawnNpcs) -> this.spawnNpcs = spawnNpcs)));
      CyclingButtonWidget<Boolean> cyclingButtonWidget5 = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.forceGameMode).build(this.column1_x, row(9), this.column2_x, 20, Text.translatable("mco.configure.world.forceGameMode"), (button, forceGameMode) -> this.forceGameMode = forceGameMode));
      CyclingButtonWidget<Boolean> cyclingButtonWidget6 = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.commandBlocks).build(i, row(9), this.column2_x, 20, Text.translatable("mco.configure.world.commandBlocks"), (button, commandBlocks) -> this.commandBlocks = commandBlocks));
      if (this.worldType != RealmsServer.WorldType.NORMAL) {
      ...
      this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.buttons.done"), (button) -> this.saveSettings()).dimensions(this.column1_x, row(13), this.column2_x, 20).build());
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawText(this.textRenderer, (Text)EDIT_SLOT_NAME, this.column1_x + this.column2_x / 2 - this.textRenderer.getWidth((StringVisitable)EDIT_SLOT_NAME) / 2, row(0) - 5, -1, false);
   ...
   class SettingsSlider extends SliderWidget {
      ...
      public SettingsSlider(final int x, final int y, final int width, final int value, final float min, final float max) {
         super(x, y, width, 20, ScreenTexts.EMPTY, (double)0.0F);
         this.min = (double)min;
         this.max = (double)max;
      ...
      public void applyValue() {
         if (RealmsSlotOptionsScreen.this.spawnProtectionButton.active) {
            RealmsSlotOptionsScreen.this.spawnProtection = (int)MathHelper.lerp(MathHelper.clamp(this.value, (double)0.0F, (double)1.0F), this.min, this.max);
         }
      }
