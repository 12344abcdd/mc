...
public class WindowFramebuffer extends Framebuffer {
   ...
   private Size findSuitableSize(int width, int height) {
      ...
      Attachment attachment = WindowFramebuffer.Attachment.NONE;

      for(Size size : WindowFramebuffer.Size.findCompatible(width, height)) {
   ...
   private boolean supportsColor(Size size) {
      ...
      GlStateManager._bindTexture(this.colorAttachment);
      GlStateManager._texImage2D(3553, 0, 32856, size.width, size.height, 0, 6408, 5121, (IntBuffer)null);
   ...
   private boolean supportsDepth(Size size) {
      ...
      GlStateManager._getError();
      GlStateManager._bindTexture(this.depthAttachment);
      GlStateManager._texImage2D(3553, 0, 6402, size.width, size.height, 0, 6402, 5126, (IntBuffer)null);
      return GlStateManager._getError() != 1285;
   }
