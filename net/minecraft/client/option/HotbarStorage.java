...
public class HotbarStorage {
   ...
   private void load() {
      try {
         ...
         for(int j = 0; j < 9; ++j) {
            this.entries[j] = (HotbarStorageEntry)HotbarStorageEntry.CODEC.parse(NbtOps.INSTANCE, nbtCompound.get(String.valueOf(j))).resultOrPartial((error) -> LOGGER.warn("Failed to parse hotbar: {}", error)).orElseGet(HotbarStorageEntry::new);
         }
