...
public class SoundManager extends SinglePreparationResourceReloader {
         ...
         try {
            for(Resource resource : resourceManager.getAllResources(Identifier.of(string, "sounds.json"))) {
               ...
               try {
                  ...
                  try {
                     profiler.push("parse");
                     Map<String, SoundEntry> map = (Map)JsonHelper.deserialize(GSON, reader, TYPE);
   ...
   static {
      ...
      GSON = (new GsonBuilder()).registerTypeHierarchyAdapter(Text.class, new Text.Serializer(DynamicRegistryManager.EMPTY)).registerTypeAdapter(SoundEntry.class, new SoundEntryDeserializer()).create();
      TYPE = new TypeToken() {
      };
   }
   ...
   protected static class SoundList {
                  ...
                  soundContainer = new SoundContainer() {
                     ...
                     public Sound getSound(Random random) {
                        ...
                        if (weightedSoundSet == null) {
                           ...
                           return new Sound(sound.getIdentifier(), new MultipliedFloatSupplier(new FloatSupplier[]{sound.getVolume(), sound.getVolume()}), new MultipliedFloatSupplier(new FloatSupplier[]{sound.getPitch(), sound.getPitch()}), sound.getWeight(), Sound.RegistrationType.FILE, sound.isStreamed() || sound.isStreamed(), sound.isPreloaded(), sound.getAttenuation());
                        }
                     }
      ...
      public void reload(Map sounds, Map soundResources, SoundSystem system) {
         ...
         for(Map.Entry entry : this.loadedSounds.entrySet()) {
